...
- **Streamlit Management** — right-click any schema and choose **Create Object** → **Projects** → **Streamlit…**, or right-click an existing Streamlit app (listed under the **Streamlits** group) to manage interactive Python data apps:
  ...
  - **New from template** — from either the schema's **Create Object → Projects** submenu or the **Streamlits** group, choose **New Streamlit app from template…** to scaffold a starter app from the [Snowflake-Labs/snowflake-demo-streamlit](https://github.com/Snowflake-Labs/snowflake-demo-streamlit) demos (Apache-2.0). Browse the catalog (fetched live, with a searchable name + description list and a built-in fallback when GitHub is unreachable), pick a destination folder, and the chosen app is downloaded there — only that folder's files, plus the Apache-2.0 `LICENSE` and a `NOTICE` provenance line. The picker shows a visible credit to the source repo. On success you can **Deploy now** — which opens the deploy dialog pre-filled with the scaffolded folder and its auto-detected main file — or **Open folder** to inspect it first.

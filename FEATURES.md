...
- **Live SQL diagnostics** — squiggly-line markers appear 400 ms after each edit and clear automatically once fixed; no false positives on well-formed Snowflake SQL:
  ...
  - **Grammar warnings** (yellow squiggly) — Snowflake-dialect PEG parser checks full grammatical structure of SELECT, INSERT, UPDATE, CREATE, DROP, ALTER, and related statements; Snowflake-specific constructs unsupported by the parser (such as CREATE STREAM) are validated via Go-side regex patterns to avoid false positives

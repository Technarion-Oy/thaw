...
- **Expand `*`** — right-click a select-list wildcard and choose **Expand \*** to replace it with the table's column list. `alias.*` expands only that table's columns (each kept `alias.`-prefixed); a bare `*` over a single `FROM` table expands unqualified; a bare `*` over multiple tables expands every table's columns, each prefixed with its alias-or-name. Columns come from the same metadata call as drag-and-drop; `COUNT(*)` and other function-argument stars are skipped, and the menu item only appears when the cursor is on a wildcard

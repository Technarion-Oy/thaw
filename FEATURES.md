...
- **File Watching** — open **View → File Watching…** to tune the file-system watcher (the one that auto-refreshes the File Browser and reloads externally edited tabs) for large or atypical workspaces such as network drives or huge dependency trees. Changes apply immediately — the watcher restarts on save, no app restart needed:
  - **Exclude patterns** — glob patterns whose changes are ignored (no re-list, no tab reload). A single name (e.g. `node_modules`) excludes that directory anywhere in the tree; a slashed pattern (e.g. `.git/objects`) excludes that subtree. Sensible defaults ship pre-filled (`node_modules`, `venv`, `.venv`, `__pycache__`, `dist`, `build`, `target`, `.git/objects`, `*.dist-info`) and are fully editable; **Reset to Defaults** restores them.

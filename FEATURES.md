...
- **Enable / Disable / Drop** users with a single right-click action, plus quick actions for **Reset password…** (`ALTER USER … RESET PASSWORD` — generates a single-use reset URL), **Rename…** (`RENAME TO`, with a dialog that explains bare vs. quoted casing), and **Abort all queries…** (`ABORT ALL QUERIES` — cancels every running/queued query across the user's sessions)

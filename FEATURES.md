...
- **Unsaved-change indicator** — a `•` dot in the tab's trailing slot (where the ✕ appears on hover, VS Code style) shows unsaved work at a glance without nudging the title sideways; a `↺` in the same slot marks a tab whose file was deleted from disk
...
- **Active Files dropdown** — a downward-triangle button pinned at the far-right of the tab strip (after the `+`) opens a searchable list of every open tab — icon, dirty `•` / orphan `↺` prefix, and title; click an entry to jump to that tab; filter by title; the active tab is highlighted; open it with `⌘⇧E` / `Ctrl+Shift+E` and dismiss with Escape or an outside click; stays visible even when many tabs overflow the bar. Each row has a hover close (×) button and a right-click context menu with the same actions as the strip's — Rename, Close, Close Others / to the Right / Saved / All, and Split with — so tabs that have scrolled off the bar can still be closed, renamed, or split without hunting for them; hovering a row (or a tab in the strip) whose title is truncated reveals a tooltip with the full title — the backing file's full path for file tabs — so files from the same directory are distinguishable, while names that fully fit show no tooltip
- **Self-describing scratch tabs** — a new scratch tab starts as `SQL 1`, `SQL 2`, … but as soon as you type, its title describes what it holds — `SELECT · ORDERS`, `CREATE TABLE · CUSTOMERS`, `SHOW WAREHOUSES` — derived from the first statement, so a strip of open queries reads as five different things instead of five numbered placeholders; the derived title is provisional and disappears the moment you rename the tab or save it to a file (the Active Files list, its filter, and the **Split with** menu all show the same name). Closing the last remaining tab replaces it with a fresh `SQL 1` (the numbering resets rather than climbing)
- **Calm tab strip** — plain query tabs carry no icon, so the file, diff, notebook, and MCP icons actually mean something; tabs are separated by short hairlines rather than full-height rules, leaving the active tab's underline as the one strong signal; and when tabs overflow the bar, the clipped edge fades out and the Active Files button shows how many tabs are open
- **Keyboard-navigable tabs** — the tab strip is a real ARIA tablist: Tab moves focus into it, ← / → step between tabs (Home / End jump to the ends), and the focused tab is the active one
...
- **Preview tabs** — like VS Code, a single click on a file in the file browser (or a search result) opens it in a reusable *preview* tab, shown with an *italic* title; clicking another file replaces its content instead of piling up tabs, so you can browse quickly without flooding the tab strip. The tab becomes permanent (normal font) as soon as you **double-click** the file (or the tab), or **edit** its contents; a preview tab with unsaved edits is never replaced. Toggle the behavior off with **Editor Preferences → Tabs → "Single-click opens a preview tab"** to always open permanent tabs
...
- **Advanced search** — the sidebar search box (focus it with `⌘⇧F` / `Ctrl+Shift+F`) searches objects **account-wide**. Pick one or more **object types** (Tables, Views, Procedures, Tasks, Stages, …) to search — the type is required so every search stays fast: it runs one `SHOW <kind> IN ACCOUNT` query per selected kind, independent of how many databases/schemas you have (finding a single Streamlit is one query, not a walk of the whole account; searching *every* kind would mean ~45 broad metadata queries, so it's scoped by type instead). Refine with a **regex** toggle (`.*` — invalid patterns show an error state and fall back to a literal match instead of throwing), a **match-case** toggle (`Aa`), and the text box (leave it empty to list everything of the selected type(s); type to narrow). Substring queries are filtered on the server; regex is applied client-side. Results are grouped by database → schema → type, expanding a matched table/view/stage/git-repo shows its real columns/files, the query is debounced so it doesn't fire on every keystroke, dropping/creating/renaming an object re-runs the search so results stay live, and clearing the search restores whatever you had manually expanded before searching
- **Multi-selection** — hold `⌘` (macOS) or `Ctrl` (Windows/Linux) and click anywhere on an object row to select it, or `⇧`+click to select a whole range; selected rows are highlighted across the full width of the sidebar; click any non-modifier area to clear the selection. **Database rows support the same gestures**; object and database selections are mutually exclusive (picking a database clears an object selection and vice versa), so the bulk actions are never ambiguous. A modified click only selects — it no longer expands the row, so building a selection doesn't trigger a load for every row it touches. The **bulk entries appear only when you right-click a row that is part of the selection** — right-clicking anywhere else (another database, a schema, a column, a git or stage row) leaves the selection untouched and shows only the ordinary single-row actions, so a menu never offers two similar-looking actions aimed at different rows
- **Batch deletion** — when multiple objects are selected, right-click any *selected* object and choose **Delete N selected objects…** to drop all of them in one operation; a confirmation dialog lists all objects to be removed, and the batch always closes with a single summary — dropped N, dropped N of M, or dropped 0 of M when every one failed
- **Batch database actions** — when multiple databases are selected, right-click any *selected* database for **Export DDL for N selected databases…** (opens the DDL export dialog with all of them pre-selected) and **Drop N selected databases…**, whose confirmation lists every database with its own Time Travel retention window — a database with no retention is flagged in red as an irreversible drop — plus the same **CASCADE / RESTRICT** mode selector as the single-database drop. Both drops show a progress toast while the per-database retention windows are fetched, so the click registers visibly before the confirmation opens, and each batch ends with one summary toast (an all-failed batch reports as an error rather than only per-item messages)
...
- **External Function Management** — right-click any schema and choose **Create Object** → **External Function…**, or right-click an existing external function (listed under the **External Functions** group) for external-function-specific operations. External functions are UDFs that call code executed *outside* Snowflake (AWS Lambda, Azure / GCP functions) by proxying an HTTPS request through an **API integration**:
  ...
  - **DDL Export / Dependencies** — `GET_DDL('FUNCTION', '<fqn>(<args>)')` powers the hover DDL preview, View Definition, comparison, and database DDL export; **Dependencies & References…** is available like regular UDFs
...
- **Centralized Tag Management** — open **Tools → Tag Management…** (or right-click the **Tags** group and choose **Manage Tags…**) for an account-wide governance view of every tag and where it is applied, instead of inspecting tags one schema at a time:
  ...
  - **Edit in place** — change a tag's value inline or remove a tag from any reference row (`ALTER <object> SET / UNSET TAG`), and **Apply tag…** to add a tag to a new object by picking a tag from the catalog, the object's domain, and then cascading **database → schema → object → column** dropdowns (no manual typing — populated live from the account, with `INFORMATION_SCHEMA` excluded; account-level domains such as warehouses/roles take a typed name) plus the value. The domain list covers every taggable object kind (tables, columns, views, the specialized table kinds, functions/procedures, schemas, databases, stages, streams, tasks, pipes, sequences, alerts, streamlits, services, notebooks, models, semantic views, every policy kind, warehouses, roles, users, …), and an **Other…** entry reveals a free-text domain field so any taggable domain Snowflake supports can be targeted. When the selected tag declares **allowed values**, the value field (both here and in the inline reference-row editor) becomes a dropdown restricted to those values
...
- **Tags in every Properties modal** — the **Properties** dialog of every taggable object kind (tables and all specialized table kinds, views/materialized views, functions/procedures/external & data-metric functions, sequences, stages, streams, tasks, alerts, pipes, streamlits, services, image repositories, network rules, contacts, datasets, models, model monitors, semantic views, Cortex search services, agents/external agents, every policy kind, warehouses, plus databases/schemas/users) now has an inline **Tags** section that lists the object's current tags and lets you add/remove them in place (`ALTER <object> SET / UNSET TAG`). Current tags are read live (no propagation latency) from `INFORMATION_SCHEMA.TAG_REFERENCES`; tags inherited from a container (schema/database/account) are shown for context but can only be unset where they were applied. The tag-name field is a searchable dropdown of the account tag catalog, and when the chosen tag declares **allowed values** the value field is restricted to that whitelist
...
- **Streamlit Management** — right-click any schema and choose **Create Object** → **Projects** → **Streamlit…**, or right-click an existing Streamlit app (listed under the **Streamlits** group) to manage interactive Python data apps:
  ...
  - **Deploy local Streamlit** — from the schema's **Create Object → Projects** submenu or the **Streamlits** group, choose **Deploy local Streamlit…** to stand up a Streamlit app from a folder on your machine in one action. A native folder picker selects the app directory; the **main file** is auto-detected (`streamlit_app.py` / `app.py`, else you pick from the folder's `*.py` files). Fill in name (defaulting to the folder name), OR REPLACE, an optional query warehouse, title, and comment. On deploy the folder's files are uploaded to a temporary internal stage — recursively, preserving subdirectories, skipping `.git/`, `__pycache__/`, virtualenv/dependency trees (`venv/`, `env/`, `node_modules/`), other hidden files, and `.DS_Store`, but keeping `.streamlit/` so `config.toml` theming ships with the app — then `CREATE [OR REPLACE] STREAMLIT … FROM @stage MAIN_FILE = …` runs and the temporary stage is dropped. (Streamlit copies files once at create time, so the temporary stage need not persist.) Symlinks are followed only *within* the app folder: a link pointing outside it is skipped rather than uploaded, so nothing from elsewhere on your disk can end up in the app. Subfolder names containing spaces upload correctly.
  - **Redeploy from local folder** — right-click an existing app and choose **Redeploy from local folder…** to update it from your machine: the same dialog opens fixed to that app with `OR REPLACE` enforced. Because Streamlit snapshots files at create time, a running app is refreshed by re-uploading to a fresh temporary stage and issuing `CREATE OR REPLACE STREAMLIT` (a plain re-upload alone would not refresh it).
  - **New from template** — open **Tools → New Streamlit App from Template…** in the menu bar (it's a local scaffolding workflow, not an object-store action) to scaffold a starter app from the [Snowflake-Labs/snowflake-demo-streamlit](https://github.com/Snowflake-Labs/snowflake-demo-streamlit) demos (Apache-2.0). Browse the catalog (fetched live, with a searchable name + description list and a built-in fallback when GitHub is unreachable), pick a destination folder (the picker opens in the currently-opened folder, and its **New Folder** button lets you create a fresh empty target in place), and the chosen app is downloaded there — only that folder's files, plus the Apache-2.0 `LICENSE` and a `NOTICE` provenance line. The picker shows a visible credit to the source repo. On success you can **Open folder** to inspect it, or — when a database/schema is active — **Deploy now**, which opens the deploy dialog pre-filled with the scaffolded folder and its auto-detected main file (targeting the active schema).
  - **Local preview** — inside the deploy dialog, **Preview locally** runs `streamlit run` on the selected folder using your Snowpark Python environment and opens it in your browser, so you can eyeball the app before pushing it. Stop it from the same control. A caveat is shown: Snowflake's Streamlit runtime pins specific Python/Streamlit versions and an allow-listed Anaconda package set, so "runs locally" ≠ "runs in Snowflake".
...
- **Dependencies & References…** — right-click **any** object in the browser and choose **Dependencies & References…** to open a unified two-tab modal that surfaces both directions of the object graph consistently, regardless of object kind:
  - **Depends on** tab (downstream — what this object uses):
    - **Parsed from DDL** — for views, procedures, and functions, a recursive dependency tree built live by parsing the object's DDL (instant, no extra privileges). Every referenced object appears with its kind icon, colour-coded type tag, and fully-qualified name; the tree is recursive up to 8 levels deep and fully expanded on load (nodes collapse/re-expand manually). **Circular reference detection** marks objects already shown higher in the tree with an "already shown" badge; tables and non-SQL bodies are leaf nodes; **multi-CTE correctness** excludes all `WITH`-clause CTE aliases so only real object references become nodes
    - **From ACCOUNT_USAGE** — an on-demand section backed by `SNOWFLAKE.ACCOUNT_USAGE.OBJECT_DEPENDENCIES` that covers the kinds the parser cannot read (tables, non-SQL procedure/function bodies). Loads automatically for non-parseable kinds; behind a **Load** button for parseable kinds since the instant tree already answers. Clearly labelled as requiring governance privileges (a grant on the `SNOWFLAKE` database / ACCOUNTADMIN) and subject to propagation latency, and degrades gracefully to a warning when the privilege is missing
  - **Referenced by** tab (upstream — what uses this object) — an on-demand flat list from `SNOWFLAKE.ACCOUNT_USAGE.OBJECT_DEPENDENCIES` (reverse direction), auto-loaded on first visit with the same privilege/latency labelling and graceful degradation
  - **Hover for DDL** — hovering any tree node (or non-routine ACCOUNT_USAGE list row) shows its DDL definition in a tooltip; content is fetched lazily on first hover and cached for 60 seconds. Procedure/function rows in the ACCOUNT_USAGE lists omit the hover (and say so in a footnote) because `OBJECT_DEPENDENCIES` doesn't record the argument signature `GET_DDL` needs
  - The policy / tag / data-metric-function **References** sections inside their respective **Properties…** modals (from the specialized `POLICY_REFERENCES` / `TAG_REFERENCES` / `DATA_METRIC_FUNCTION_REFERENCES` views) remain in place — they surface the *applied-to* relationship that `OBJECT_DEPENDENCIES` does not capture
...
- **Create Table** — right-click any schema and choose **Create Object** → **Tables & Views** → **Table…** to open a comprehensive table designer:
  ...
  - **Column Editor** — dynamic list of columns:
    ...
    - Set **Default Values** (free text, or pick a built-in function like `CURRENT_TIMESTAMP()` / `UUID_STRING()` from the **ƒ** shortcut — a **searchable** dropdown that groups every function valid as a column default by category: Date & Time, Session & Context, Identifiers & Misc) and **Comments** for each column
...
- **Time Travel / Undrop** — a **Show Dropped Objects…** view lists every object recoverable via Time Travel within its retention window, **grouped by object kind**, each row showing the name and its dropped-on timestamp with a one-click **Undrop**. Three scopes cover the enumerable kinds: right-click a **schema** for dropped **tables** and **iceberg tables** (restored with the correct `UNDROP TABLE` / `UNDROP ICEBERG TABLE`), right-click a **database** for dropped **schemas**, and the Objects-panel toolbar button for dropped **databases**. Kinds Snowflake provides no dropped-object listing for — dynamic tables, tags, streamlits, notebooks, and external volumes (none expose a `SHOW … HISTORY` form with a `dropped_on` column) — are out of scope; a dropped object of one of those kinds can still be restored by running its `UNDROP` statement in the editor if you know its name.
- **ER Diagram** — generate an Entity Relationship Diagram for any database; filter by schema, zoom, pan, and copy the Mermaid source. The canvas culls off-screen nodes/edges from the DOM and offers a **Minimap** toggle (remembered between sessions) to drop the minimap's second copy of every node — both trim memory use on wide schemas, notably under Windows/WebView2
...
- **File Watching** — open **View → File Watching…** to tune the file-system watcher (the one that auto-refreshes the File Browser and reloads externally edited tabs) for large or atypical workspaces such as network drives or huge dependency trees. Changes apply immediately — the watcher restarts on save, no app restart needed:
  - **Exclude patterns** — glob patterns whose changes are ignored (no re-list, no tab reload). A single name (e.g. `node_modules`) excludes that directory anywhere in the tree; a slashed pattern (e.g. `build/generated`) excludes that subtree. Hidden directories (dot-prefixed) are always skipped, so the defaults list only non-hidden directories: `node_modules`, `venv`, `__pycache__`, `dist`, `build`, `target`, `*.dist-info`. Fully editable; **Reset to Defaults** restores them.
  - **Max watched directories** — caps the number of distinct directories that emit change events (0 = unlimited); beyond the cap, changes in not-yet-seen directories are ignored, bounding resource use on pathological trees.
  - **Raise file-descriptor limit on startup** — opt-in toggle that bumps the process file-descriptor soft limit toward the hard limit when the watcher starts (a macOS/Linux mitigation; no effect on Windows).
  - Watcher setup failure is **non-fatal**: a folder always opens even if watching can't be fully established — only the auto-refresh convenience is lost.

### Configuration

Open **Tools → Configure AI Inline Completions…** in the menu bar to set your provider, API key, and model. The API key is kept in your operating system's secure store — **macOS Keychain**, **Windows Credential Manager**, or the **Linux Secret Service** — never in `config.json`, and is never transmitted anywhere other than the selected AI provider. Each secret field in Settings shows an indicator of where it is actually stored, falling back to a warning-styled note when no OS secure store is available (a `0600` local file is used instead). See **Secure secret storage** below.
...
- **File Browser** — browse the working directory in the sidebar; click any file to open it; auto-refreshes after a DDL export; **file system watcher** monitors the working directory for external changes (files created, renamed, deleted, or edited in the terminal, other editors, or via git) and incrementally refreshes only the affected directories — no manual reload needed; an open editor tab whose file is changed externally re-reads the new content automatically (a tab with unsaved edits keeps your changes, VSCode-style, rather than overwriting them); tunable via **View → File Watching…** (exclude globs, a directory cap, FD-limit raising — see the File Watching preferences below), with IT-admin policy able to disable it entirely; **multi-select** with **Cmd/Ctrl+click** (toggle a node) and **Shift+click** (select a range), so Cut/Copy/Delete and git Stage/Unstage/Discard can act on the whole selection at once; right-click any file or folder to access the context menu:
  ...
  - **Rename…** — renames the file or folder inline in the tree (VS Code-style), with the same live name validation as creation: an invalid character or a name already taken is flagged under the input while you type, instead of the name being silently mangled or thrown away
  ...
  - **New File…** (directories only) — creates an empty file of **any type** and opens it in the editor; whatever extension you type is kept (`schema.yml`, `README.md`, `.gitignore`), and a bare name with no extension gets `.sql` appended, so the common case is still one word
  - Both creation actions work **inline in the tree, VS Code-style** — no dialog: an editable placeholder row appears under the target folder (expanding and loading it first if needed) at the position the item will sort into. **Enter** creates, **Escape** cancels, clicking away creates what you typed (or cancels if the name is empty or invalid). The name is validated **while you type** — empty names, path separators, the Windows-invalid characters `: " * ? < > |`, Windows reserved device names (`CON`, `NUL`, `LPT1`, …), and a name already used by a sibling are flagged under the input instead of failing after the fact. The Windows rules apply on every platform by design — these files live in a git repo that may be checked out on Windows, so a name that breaks there is refused on macOS and Linux too
  - **Create at the root** — **right-click the header title area** (top-left) or the **empty area** of the panel to open a root context menu (New Folder…, New File…, Paste) that targets the **workspace root** — so folders and files can be created at the top level, not only nested inside an existing folder
...
- The object browser's **Export DDL…** context-menu action (on a single database or a multi-database selection) opens the same dialog with those databases pre-selected — every export goes through the options dialog, none run straight to disk
...
- **Placeholder insert buttons** — both the Export Path Format dialog and the export dialog's per-export template override show one clickable tag per placeholder (with its description); clicking inserts the placeholder **at the cursor**, replacing the selected text, and leaves the caret after it, so a template can be built in any order — clicking with the field unfocused appends instead
- **`.sql` extension enforced** — a template that does not end in `.sql` is flagged inline and blocks the dialog: **Save** is disabled in Export Path Format, **Export** is disabled in the export dialog (a blank template is fine — it uses the default, which already ends in `.sql`). The extension is never appended for you, so a template that already carries it can't turn into `something.sql.sql`
- **Configurable overload naming** — the same **Tools → Export Path Format…** dialog chooses how overloaded functions/procedures (same name, different argument signatures) are written: **Argument types** (`FOO__VARCHAR.sql` — the default, size qualifiers dropped), **Full signature** (`FOO__VARCHAR_16.sql` / `FOO__VARCHAR_256.sql` — size qualifiers kept, so size-only overloads never collide), or **One file per name** (`FOO.sql` holding every overload, ordered by signature); persisted across sessions alongside the path template
- **Dollar-quoted function/procedure bodies** — an export-dialog toggle rewrites `FUNCTION` / `PROCEDURE` bodies from the single-quoted string literal `GET_DDL` returns (`AS 'begin … let x := ''hello''; … end'`, with doubled quotes and backslash escapes) into the non-dynamic `AS $$begin … let x := 'hello'; … end$$` form, so the exported file holds readable, syntax-highlightable code. The delimiter is chosen so the body can never terminate it early (`$$` → `$thaw$` → `$thaw_body$`); bodies that are already dollar-quoted, and the rare literal whose escapes would not survive the round trip (a `\0`, `\b`, or a numeric escape for a non-ASCII byte), are written exactly as Snowflake returned them
- **Deterministic file names** — names depend only on the objects exported, never on the order Snowflake returns them in: when two overloads do collide (two `VARCHAR` overloads under *Argument types*) the numeric `_2` / `_3` suffix is assigned by signature order, so re-exporting the same database produces the same files and a Git working directory shows no spurious changes
- **Export options** — in the dialog: pick the **output directory**, the **databases** from a multi-select dropdown (leave empty for all databases), restrict to specific **schemas** via a dropdown (suggestions list the schemas of every database in the export as qualified `DATABASE.SCHEMA` entries so same-named schemas stay individually selectable; a typed bare name matches that schema in every database, case-insensitively; leave empty for all schemas), choose which **object types** to include (type and schema selection are post-fetch filters — `GET_DDL` always returns the whole database), override the **file path template** (with the same placeholder insert tags and live example-path preview as the Export Path Format dialog) and the **overloaded function/procedure naming** for this export only (both prefilled from the saved Export Path Format settings), turn **dollar-quoted bodies** on or off for functions and procedures, choose **overwrite vs. skip** existing files (skipped files are counted in the summary), and pick the **warehouse** to run the export on (`USE WAREHOUSE` is issued for the export and the previous warehouse restored afterwards; leave empty for the session warehouse)
...
- **Git status in the Files panel** — git is folded into the file explorer (no separate Git panel). The Files header uses **two rows**: the folder title + action icons on top, and a dedicated **git status row** below it (in a repo) with a single segmented **status pill** — branch · ↑commits-to-push · changed-file total — that opens Git Operations, with the full staged/changed breakdown in its tooltip. The branch name has room to show in full instead of being squeezed into the action strip, and the changed count is accent-tinted rather than amber (uncommitted work is the normal state of an active day, not a warning). The action icons are **revealed on hover/focus** of the header, VS Code style, so the resting state is just the folder name. Both **files and folders are colour-coded by git status** — green for new/untracked (`A`/`U`), orange for modified (`M`, and folders with mixed changes), red for deleted (`D`) — with a single-letter sigil on files; folders take the aggregate colour of the changes beneath them. Coverage is uncapped, so even very large change sets colour the whole tree. Right-click a changed file to **Stage / Unstage / Discard** it
...
- **Enable / Disable / Drop** users with a single right-click action, plus quick actions for **Reset password…** (`ALTER USER … RESET PASSWORD` — generates a single-use reset URL, shown in a copyable dialog to hand to the user), **Rename…** (`RENAME TO`, with a dialog that explains bare vs. quoted casing), and **Abort all queries…** (`ABORT ALL QUERIES` — cancels every running/queued query across the user's sessions)
- **Access policies** — from **User Properties**, managed like tags: the **authentication / password / session** policies currently attached to the user show as **removable chips** (read live from `POLICY_REFERENCES`, since `DESCRIBE USER`/`SHOW USERS` omit them — click the × to `UNSET … POLICY`), and a new one is added by picking a **kind** and a **policy** from searchable dropdowns (`SHOW … POLICIES IN ACCOUNT`). A kind allows one policy, so a **FORCE** toggle replaces a same-kind policy already attached (`SET { AUTHENTICATION | PASSWORD | SESSION } POLICY … [FORCE]`)
- **Tags** — from **User Properties**, the shared object-store tag editor: current tags applied to the user render as **removable chips** (click the × to `UNSET TAG`, read live from `INFORMATION_SCHEMA.TAG_REFERENCES`), and a new tag is added by choosing its name from a **dropdown** of account tags plus a value (`SET TAG <name> = '<value>'`) — when the chosen tag defines **allowed values**, the value field is itself a dropdown restricted to that whitelist, so a rejected value can't be submitted
- **MFA method removal** — from **User Properties** (Security section), the user's **enrolled MFA factors** are listed (`SHOW MFA METHODS FOR USER`, showing type / label / last-used per factor); **Remove** deletes one factor by its system-generated identifier (`REMOVE MFA METHOD <name>`) so the user can re-enroll — distinct from the `MINS_TO_BYPASS_MFA` bypass window. Listing requires `ACCOUNTADMIN`; without it the row shows a faint "unavailable" note instead of an error
- **Delegated authorization** — from **User Properties**, let a security integration act on the user's behalf for a role, both chosen from **dropdowns** (roles from `SHOW ROLES`, integrations from `SHOW … SECURITY INTEGRATIONS`): `ADD DELEGATED AUTHORIZATION OF ROLE … TO SECURITY INTEGRATION …`, and remove one role's authorization or every delegated authorization for an integration by leaving the role empty (`REMOVE DELEGATED { AUTHORIZATION OF ROLE … | AUTHORIZATIONS } FROM SECURITY INTEGRATION …`)
...
- **Export results** — CSV (RFC 4180) and Excel (`.xlsx`) export with a native save dialog; CSV always exports whichever result is currently selected in the history dropdown. **Excel** exports the currently selected result when only one has been run; once the tab holds two or more results, the Excel button opens a picker to select any subset of the history and export them into a **single workbook with one sheet per resultset** — sheet names are derived from each query (whitespace-collapsed, capped at Excel's 31-char limit, invalid characters `[ ] : * ? / \` stripped, leading/trailing apostrophes removed, Excel's reserved `History` name avoided, and de-duplicated with numbered suffixes; empty queries fall back to `Result N`)
...
- **Authentication methods** — the connect dialog supports every interactive and non-interactive authenticator offered by the gosnowflake driver, with form fields that show/hide reactively based on the selection:
  - **Password + MFA push** — password with a push notification approval. Because a one-time MFA code/TOTP passcode can't be reused across connections, Thaw handles MFA (and password + TOTP) specially:
    ...
    - **Guidance** — the connect dialog shows an info-level note that Thaw caches the MFA token so MFA works reliably out of the box, and **recommends** enabling `ALLOW_CLIENT_MFA_CACHING = TRUE` for the smoothest sign-in with fewer prompts (shown for the MFA **or** password authenticator); when that parameter is confirmed off (MFA authenticator only), a one-time dismissible post-connect hint recommends it. Key-pair auth is offered throughout as a prompt-free alternative needing no account change.
...
- **Execution modes** — three modes control what a session can do:
  ...
  - **Read-Only SQL** — SQL execution via `execute_snowflake_sql`. Every statement passes through the EXPLAIN precompilation gate (only read-only operations allowed). This is also the only mode that exposes `deploy_streamlit`, the one tool that **writes** to Snowflake (it creates a stage and a STREAMLIT object), and only when a workspace root is configured.
...
- **Copy Config** — each running session offers a one-click copy of the client configuration block. The session's auth token is carried in an `Authorization: Bearer` header (kept out of the URL, so it does not leak into local proxy logs, process listings, or shell history), so the copied block is a **secret** — treat it like a password:
  ```json
  { "mcpServers": { "thaw-<label>": { "type": "sse", "url": "http://127.0.0.1:<port>/sse", "headers": { "Authorization": "Bearer <token>" } } } }
  ```
...
- **Streamlit tools** — generate `CREATE STREAMLIT` DDL (`build_create_streamlit_sql`), list the app templates in [Snowflake-Labs/snowflake-demo-streamlit](https://github.com/Snowflake-Labs/snowflake-demo-streamlit) (`list_streamlit_templates`), scaffold one into a local folder together with its Apache-2.0 LICENSE and a NOTICE (`create_streamlit_from_template`, workspace-gated), and deploy a local app folder to Snowflake (`deploy_streamlit`). Both template tools return the upstream repository, link, and license alongside the results, so an MCP client can show the same credit Thaw's own template picker does. `create_streamlit_from_template` also reports the detected entrypoint, so a client can scaffold and then deploy without a separate detection step.
- **`deploy_streamlit` — the one mutating MCP tool** — every other tool reads, generates SQL for you to run, or writes into your workspace folder; this one uploads an app folder to a temporary stage and runs `CREATE STREAMLIT`. It is therefore gated twice: **Read-Only SQL mode only** (Explain Only promises nothing is executed, so it is excluded) and only when a workspace root is configured. The target database, schema, and name must be given explicitly — they are never taken from the session's current context — overwriting an existing app requires opting in to `OR REPLACE`, the app folder must sit inside the workspace root, and the executed `CREATE STREAMLIT` statement is returned in the result so you can see exactly what ran.
...
- **Security** — the listener binds only the loopback interface and rejects requests with a non-loopback `Host` header or a cross-origin `Origin` header, defending against DNS-rebinding attacks from a malicious web page. Each session additionally has a **per-session auth token** (crypto-random) required to open the SSE connection, presented as an `Authorization: Bearer` header (what generated configs use, keeping the token out of the URL) or a `?token=…` query parameter (fallback for URL-only clients) — so another local process cannot read schema metadata without the token from the copied config. Rejected (unauthenticated) connection attempts are logged (rate-limited to once per 10 s per session) for observability. The token defends against non-admin local users only; a local administrator can bypass it (process memory, loopback capture). For SQL execution modes, always use a scoped read-only Snowflake role for defense in depth. Sessions should be stopped when not in use.
...
- **Tools menu** — native menu bar **Tools** entry consolidates workflow tools and operational settings: **Code Snippets…**, **Tag Management…**, **Export Path Format…**, **Schema Migration…**, **Create dbt Project…**, **Git Operations…** (`⌘G`), **New Terminal** (`⌘\``), **Configure AI Inline Completions…**, **MCP Sessions…**, **Query Log** (Enable / All / User / Internal), and **Session Management…**
- **Snowpark menu** — native menu bar **Snowpark** entry provides **Check Environment…**, **Setup Environment…**, **New Notebook…**, and **Open Notebook…**
- **Connection-aware menu bar** — menu items that need a live Snowflake connection are greyed out while disconnected (on launch and after **Disconnect**), and enable themselves after a successful connect: **Tools → Tag Management… / Export Database DDL… / Schema Migration… / Create dbt Project… / MCP Sessions…** and **Snowpark → New Notebook… / Open Notebook…**. Everything that works offline stays enabled at all times — the whole **File**, **View**, and **Help** menus, plus **Code Snippets…**, **Export Path Format…**, **New Streamlit App from Template…** (local scaffolding), **Git Operations…**, **New Terminal**, **Configure AI Inline Completions…**, **Query Log**, **Session Management…**, and the Snowpark **Check/Setup Environment…** and **Notebook Preferences…** items
- **Help menu** — **About Thaw…** opens a dialog with the product name, version, company, and copyright; **Check for Updates…** runs an on-demand update check (see below); **Function Catalog…** opens the built-in Snowflake function reference with overload signatures and descriptions for every function; **Keyboard Shortcuts…** opens a searchable modal listing every shortcut with macOS and Windows columns
- **Update notifications** — a few seconds after startup Thaw quietly checks GitHub for a newer release (skipped for local `dev` builds). If one is available, a dismissible banner appears; clicking it — or **Help → Check for Updates…** — opens a modal with the release notes and a **Download update** button that opens the GitHub release page in your browser. The check is notification-only (no in-app download/apply). It works behind a corporate proxy by reading the **operating-system proxy settings** (WinHTTP/IE on Windows, System Settings network proxies on macOS, environment variables on Linux), retrying a direct connection if the proxy fails, and is throttled + cached so it stays well under GitHub's rate limit
- **Acknowledgements & third-party licenses** — the **About Thaw** dialog has an **Acknowledgements** button opening a searchable list of every open-source package bundled into Thaw (Go modules and npm packages) with its version, license, and full copyright/license text. The list is generated from the actual dependency tree into `THIRD_PARTY_NOTICES.md` (embedded in the binary) — regenerate with `go run scripts/gen_third_party_notices.go`
- **First-launch license agreement** — on the very first launch (and after upgrading an install that predates this gate) Thaw shows a full-screen, non-dismissible **License Agreement** displaying the GNU GPL v3 license text. **Accept** persists the choice so subsequent launches skip it and reveals the workspace; **Decline & Quit** closes the app. The gate is enforced in-app (not via OS installers), so it behaves identically across macOS, Windows, and Linux and cannot be bypassed by extracting the binary. The choice is stored as `licenseAccepted` in the config file

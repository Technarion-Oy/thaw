...
- **Authentication methods** — the connect dialog supports every interactive and non-interactive authenticator offered by the gosnowflake driver, with form fields that show/hide reactively based on the selection:
  - **Password + MFA push** — password with a push notification approval. Thaw caches the MFA token and serializes logins to avoid MFA-challenge storms. Because a one-time MFA code/TOTP passcode can't be reused across connections, an MFA session runs on a **single, long-lived connection** (one code per session) rather than a pool — eliminating the repeated-login/account-lock risk. When that connection is eventually lost (e.g. the server's ~4h session timeout), Thaw **pops up a modal asking for a fresh MFA code** and transparently reconnects (typed-TOTP), or re-triggers a device push (push MFA). The connect dialog still shows a prominent error-level warning that MFA **can lock the account** unless `ALLOW_CLIENT_MFA_CACHING = TRUE` (shown for the MFA **or** password authenticator), and when the parameter is confirmed off Thaw shows a one-time, dismissible post-connect hint recommending it. Key-pair auth is offered throughout as a prompt-free alternative that needs no account change. Trade-off: an MFA session runs queries and bulk actions (e.g. DDL export) serially on that one connection — editor tabs share it rather than opening their own logins (so per-tab session isolation is disabled), and MCP sessions, which need an independent connection, are unavailable under single-use MFA (use key-pair for those).

...
- **Right-click tables and views** to:
  ...
  - **Import Data** — upload one or more local files into Snowflake; supports CSV, JSON, AVRO, ORC, and Parquet; exposes all Snowflake `FORMAT_TYPE_OPTIONS` with defaults pre-filled; can create a new table automatically by inferring the schema; **file preview** for CSV and JSON shows the first 10 rows of up to 5 files — CSV offers **Parsed** (table respecting current delimiter and header settings) and **Raw** views; JSON offers **Parsed** (tabular, supports arrays-of-objects and NDJSON) and **Raw** views; multiple files shown in a tabbed layout; **✨ AI Suggest** button (CSV and JSON, requires AI configured) analyses the first file's content and auto-fills format options — delimiter, header detection, quoting, encoding, compression, etc. — with a one-sentence explanation; a ⓘ icon next to the button discloses that a file sample is sent to the configured AI provider

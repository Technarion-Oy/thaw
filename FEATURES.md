...
- **Batch database actions** — when multiple databases are selected, right-click any of them for **Export DDL for N selected databases** (writes each database's DDL to the working directory, reporting total files and any failures) and **Drop N selected databases…**, whose confirmation lists every database with its own Time Travel retention window — a database with no retention is flagged in red as an irreversible drop — plus the same **CASCADE / RESTRICT** mode selector as the single-database drop

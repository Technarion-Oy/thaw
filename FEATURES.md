...
- **Advanced search** — the sidebar search box (focus it with `⌘⇧F` / `Ctrl+Shift+F`) searches objects **account-wide** with three refinements: a **regex** toggle (`.*` — invalid patterns show an error state and fall back to a literal match instead of throwing), a **match-case** toggle (`Aa`), and a **type filter** — a multi-select of object kinds (Tables, Views, Procedures, Tasks, Stages, …). It runs entirely in the backend via one `SHOW <kind> IN ACCOUNT` query per kind, so it's fast regardless of how many databases/schemas you have — searching for a single Streamlit is one query, not a walk of the whole account. Substring queries are filtered on the server; regex is applied client-side. Results are grouped by database → schema → type, expanding a matched table/view/stage/git-repo shows its real columns/files, the query is debounced so it doesn't fire on every keystroke, and clearing the search restores whatever you had manually expanded before searching

...
- **Visual File Format Builder & Previewer** — right-click any schema and choose **Create Object** → **Data Loading** → **File Format…**, or right-click the **FILE FORMATS** folder to open the designer:
  ...
  - **Execution** — runs `ExecDDL` and refreshes the schema tree automatically on success
- **Right-click a database** to **Create Database…**, **Export DDL…** (opens the same DDL export dialog as **Tools → Export Database DDL…**, with that database pre-selected, so the output directory, schema/object-type filters, path template, overwrite mode and warehouse can be reviewed before anything is written), generate an ER Diagram, **Show Dropped Objects…** (schemas recoverable via Time Travel), or open **Backup Sets…**

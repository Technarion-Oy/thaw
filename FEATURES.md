...
- **MFA method removal** — from **User Properties** (Security section), remove an enrolled MFA method (`REMOVE MFA METHOD { TOTP | PASSKEY | DUO }`) so the user can re-enroll — distinct from the `MINS_TO_BYPASS_MFA` bypass window

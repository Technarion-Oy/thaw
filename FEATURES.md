...
- **Access policies** — from **User Properties**, attach or detach an **authentication**, **password**, or **session** policy (`SET / UNSET { AUTHENTICATION | PASSWORD | SESSION } POLICY`) by picking from a searchable **dropdown** of every policy of that kind in the account (`SHOW … POLICIES IN ACCOUNT`); a **FORCE** toggle replaces a same-kind policy already attached. Current assignments aren't shown because `SHOW USERS` omits them

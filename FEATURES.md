...
- **Authentication methods** — the connect dialog supports every interactive and non-interactive authenticator offered by the gosnowflake driver, with form fields that show/hide reactively based on the selection:
  - **Password + MFA push** — password with a push notification approval. Thaw caches the MFA token and serializes logins to avoid MFA-challenge storms. Because Thaw's pooled connections each re-use a one-time MFA code/TOTP passcode, MFA **can lock the account** unless the account has `ALLOW_CLIENT_MFA_CACHING = TRUE`: the connect dialog shows a prominent error-level warning to this effect whenever the MFA **or** password authenticator is selected, and when the parameter is confirmed off Thaw also keeps the connection pool small and shows a one-time, dismissible post-connect hint. Key-pair auth is offered throughout as a prompt-free alternative that needs no account change.

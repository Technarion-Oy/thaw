...
- **Live SQL diagnostics** — squiggly-line markers appear 400 ms after each edit and clear automatically once fixed; no false positives on well-formed Snowflake SQL:
  ...
  - **Grammar warnings** (yellow squiggly) — Snowflake-dialect PEG parser checks full grammatical structure of SELECT, INSERT, UPDATE, CREATE (including STREAM), DROP, ALTER, and related statements; Snowflake-specific constructs unsupported by the parser are skipped to avoid false positives

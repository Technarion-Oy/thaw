...
- **Right-click a task** to:
  ...
  - **View Task Graph…** — opens an interactive DAG visualisation of the complete task graph rooted at the selected task:
    ...
    - **Right-click any node** to Suspend or Resume that individual task; the menu shows the relevant action based on the task's current schedule state (STARTED → Suspend, SUSPENDED → Resume); issues `ALTER TASK IF EXISTS … SUSPEND/RESUME` and updates the schedule state badge on the node immediately without waiting for the next poll

...
- **View Management** — right-click any schema and choose **Create Object** → **Tables & Views** → **View…**, or right-click an existing view (listed under the **Views** group) for view-specific operations:
  ...
  - **Properties** — view `SHOW VIEWS` metadata plus inline-editable **Comment** and a **SECURE** toggle (applied via `ALTER VIEW … SET / UNSET`), and the rendered **Defining Query**
  ...
  - **Drop / Rename** — standard danger-confirmation dialog executes `DROP VIEW`; **Rename…** issues `ALTER VIEW … RENAME TO`
...
- **Git Operations Dialog** — open via **Git → Git Operations…** (`⌘G` / `Ctrl+G`), the branch chip, or the Git Operations button in the Files header:
  ...
  - **Commit** — the commit summary box commits the **staged set** (the real git index). The button is a **split button**: the main action is **Commit & Push** (names its target, e.g. *"Commit & Push 4 staged"*), and the ▾ dropdown offers **Commit (no push)** for a local-only commit. Commit-only works without a GitHub connection; push requires it. Personal-access / OAuth token is ephemeral, never saved

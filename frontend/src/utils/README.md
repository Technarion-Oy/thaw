...
| `modalDragResize.ts` | Side-effect module (imported once in `main.tsx`) that installs a single document-level `mousedown`/`mousemove`/`mouseup` delegate to make **every** Ant Design modal draggable by its `.ant-modal-header` — no per-Modal props. Position is an inline `transform` on the `.ant-modal` box; because Thaw's modals unmount on close, it resets to default on reopen. Resize is handled separately by CSS (`resize: both` on `.ant-modal-content`, in `styles/global.css`). Issue #572. |

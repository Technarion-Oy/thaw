...
| `objectDdl.ts` | `DDL_UNSUPPORTED_KINDS` set + `kindSupportsDdl(kind)` (nullish → supported) — the object kinds Snowflake's `GET_DDL` cannot render (IMAGE REPOSITORY, SERVICE, GATEWAY, PACKAGES POLICY, MODEL, MODEL MONITOR, DATASET, CORTEX SEARCH SERVICE, EXTERNAL AGENT, MCP SERVER). Mirror of the guard in `internal/snowflake/client.go GetObjectDDL`; every frontend DDL entry point checks it before firing a doomed `GET_DDL` — `Sidebar` hover tooltip, `Sidebar` **View Definition** / **Select for Comparison** / **Compare with** menu gates, `SqlEditor` cmd/ctrl-hover, and `diffStore.fetchText` (defence-in-depth). Keep in sync with the Go list. |

...
| `objectDdl.ts` | Re-exports `DDL_UNSUPPORTED_KINDS` from `src/generated/objectKinds.ts` and adds `kindSupportsDdl(kind)` (nullish → supported) — the object kinds Snowflake's `GET_DDL` cannot render (IMAGE REPOSITORY, SERVICE, GATEWAY, PACKAGES POLICY, MODEL, MODEL MONITOR, DATASET, CORTEX SEARCH SERVICE, EXTERNAL AGENT, MCP SERVER). The list is **generated** from the canonical registry `internal/objectkind` — the same source `internal/snowflake/client.go GetObjectDDL` derives its guard from, so the two cannot drift and nothing needs hand-syncing. Every frontend DDL entry point checks it before firing a doomed `GET_DDL` — `Sidebar` hover tooltip, `Sidebar` **View Definition** / **Select for Comparison** / **Compare with** menu gates, `SqlEditor` cmd/ctrl-hover, and `diffStore.fetchText` (defence-in-depth). |

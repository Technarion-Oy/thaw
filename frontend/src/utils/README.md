...
| `fieldClipboard.ts` | Exports `isMonacoCodeSurface(el)` — the single source of truth (shared by `App.tsx`'s global clipboard handler and `monacoClipboard.ts`) for "is this Monaco's own code-editing surface (`.inputarea`)" vs. an ordinary editable field (find/replace/rename) that merely lives inside `.monaco-editor`. |
| `monacoClipboard.ts` | `patchMonacoClipboard(editor)` routes the **code buffer's** copy / cut / paste through Wails' `ClipboardGetText` / `ClipboardSetText` native APIs (required because `navigator.clipboard` is blocked inside WKWebView on macOS). Its capture-phase Cmd/Ctrl+V/C/X handler acts **only** when the focused element is the code surface (`isMonacoCodeSurface`); for find/replace/rename fields it lets the event bubble to the global handler in `App.tsx`. It also calls `installFindWidgetTooltipFix()` — since every Monaco mount in the app (SqlEditor, notebook cells, modals, diff views) routes through `patchMonacoClipboard`, this is where the tooltip fix is installed. Warns once if Monaco's `.inputarea` code-surface class ever goes missing. |
| `monacoTooltipFix.ts` | `installFindWidgetTooltipFix()` — idempotent, post-mount patch of Monaco's base-layer hover service (`_createHover`) forcing find-widget button tooltips to render below their target instead of the default above, so they aren't clipped by the editor pane's `overflow: hidden` (issue #593). Patches the singleton immediately for the mounting editor (so a modal that's the first Monaco surface of a session is covered) and registers a one-time global `onCodeEditorAdd` hook for every future editor. Warns once and stops if `_createHover` is unavailable (e.g. a Monaco version bump). |

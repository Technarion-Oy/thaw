...
| `modalDragResize.ts` | Side-effect module (imported once in `main.tsx`) that installs a single document-level `mousedown`/`mousemove`/`mouseup` delegate to make **every** Ant Design modal draggable — no per-Modal props. The handle is the `.ant-modal-header` plus the content padding box (so the very top edge grabs), minus interactive controls and the bottom-right resize grip; the drag translate is clamped so the header stays on-screen, and a lost mouseup (`e.buttons === 0` / window `blur`) is recovered. Position is an inline `transform` on the `.ant-modal` box; because Thaw's modals unmount on close, it resets to default on reopen. Resize is handled separately by CSS (`resize: both` on `.ant-modal-content`, in `styles/global.css`). Issue #572. |

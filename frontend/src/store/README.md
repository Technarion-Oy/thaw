...
| `gitStore.ts` | Git repo config (exportDir, remoteURL, branch, author, `recentDirs`) and runtime state (status with `staged`/`unstaged` lists, pull/clone/staging/committing loading flags, branches list). **Working directory** = `exportDir`; `openFolder(dir)` sets it and pushes it onto `recentDirs` (newest first, deduped, cap 8, persisted in `GitConfig`), `pickExportDir` picks a directory then calls `openFolder`, `clearRecentDirs` empties the list, `openInNewWindow` picks a folder and launches a second Thaw instance there (`OpenFolderInNewInstance` IPC) without changing this window's `exportDir`. Staging actions (`stageFile`, `unstageFile`, `stageAll`, `unstageAll`, `discardFile`) operate on the real index; `commitStaged` commits the staged set and pushes. OAuth token is in-memory only, never persisted. Calls all `Git*` IPC methods directly. |

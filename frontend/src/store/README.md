...
| `objectStore.ts` | In-memory cache of databases, schemas, and objects expanded in the sidebar tree. Used as tier-1 of the search cascade. No persistence. `removeDatabase`/`removeSchema`/`removeObject` evict entries after a DROP, or when a hover's `GET_DDL` reports the object gone or unauthorized (#918); `removeObject` matches on kind too, so same-named objects in one schema are evicted independently. |

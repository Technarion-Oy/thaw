...
| `gitStore.ts` | Git repo config (exportDir, remoteURL, branch, author) and runtime state (status with `staged`/`unstaged` lists, push/pull/clone/staging/committing loading flags, branches list). Staging actions (`stageFile`, `unstageFile`, `stageAll`, `unstageAll`, `discardFile`) operate on the real index; `commitStaged` commits the staged set. OAuth token is in-memory only, never persisted. Calls all `Git*` IPC methods directly. |

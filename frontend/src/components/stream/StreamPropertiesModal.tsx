...
export default function StreamPropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const props = await GetObjectProperties(db, schema, "STREAM", name);
      setRows(props ?? []);
    } catch (e) {
      setError(String(e));
    }
  ...
  const objTags = useObjectTags({
    kind: "STREAM", db, schema, name,
    alter: (clause) => AlterStream(db, schema, name, clause),
  });

  const streamRef = `"${db}"."${schema}"."${name}"`;
  ...
  const saveComment = async (comment: string) => {
    if (comment.trim() === "") {
      await AlterStream(db, schema, name, "UNSET COMMENT");
    } else {
  ...
  return (
    <Modal
      ...
      title={
        <Space size={6}>
          <ThunderboltOutlined style={{ color: "var(--link)" }} />

...
`StreamPropertiesModal` shows `SHOW STREAMS` metadata, an inline-editable
comment, and an inline **Tags** editor (`SET / UNSET TAG`). The remaining
operations (Drop / Rename) are driven from the sidebar context menu in
`layout/Sidebar.tsx` via `App.AlterStream`.

## Files

| File | Purpose |
|------|---------|
| `CreateStreamModal.tsx` | `CREATE STREAM` form — name/options (OR REPLACE / IF NOT EXISTS), a source-type `Select` (TABLE / VIEW / EXTERNAL TABLE / STAGE / DYNAMIC TABLE), a **database → schema → object picker** for the source (`ListDatabases`/`ListUserSchemas`/`ListObjects`, schemas exclude `INFORMATION_SCHEMA` since you can't create a stream on its objects; the object dropdown filtered to the chosen source type; the picked object's quoted FQN becomes `source`), comment, and an **Advanced options** `Collapse` whose CDC flags are gated by source type (APPEND_ONLY / SHOW_INITIAL_ROWS for table/view/dynamic-table, INSERT_ONLY for external-table; COPY GRANTS always) and which are reset when the source type changes. Uses `BuildCreateStreamSql` for live SQL preview. |
| `StreamPropertiesModal.tsx` | Loads `GetObjectProperties(db, schema, "STREAM", name)`; renders an inline-editable Comment (via `AlterStream … SET/UNSET COMMENT`), an inline **Tags** editor (`TagsRow` + `GetObjectTagReferences("STREAM", …)`, applied via `AlterStream … SET/UNSET TAG`; inherited tags shown for context), and the remaining `SHOW STREAMS` properties (source_type, mode, stale, etc.). |
...
- `GetObjectTagReferences("STREAM", db, schema, name, "")` — current tags (no-latency `INFORMATION_SCHEMA.TAG_REFERENCES` read)
- `AlterStream(db, schema, name, clause)` — `SET / UNSET COMMENT`, `SET / UNSET TAG …`
...
- Streams have no lifecycle (suspend/resume) and no secure / defining-query
  surface; `ALTER STREAM` supports only `SET / UNSET COMMENT` and
  `SET / UNSET TAG`, so the properties panel exposes an editable comment and a
  tags editor plus the raw `SHOW STREAMS` metadata (rename is sidebar-only).

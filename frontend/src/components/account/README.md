...
- **No privilege pre-checks**: admin actions (create/edit/drop users, create integrations, warehouse metering) are *not* gated by up-front Snowflake privilege probes — those checks were unreliable. Every action is offered; if the current role lacks the privilege, the IPC call fails and the error surfaces via `message.error`. The backend `Can*` IPC methods (`CanCreateUsers`, `CanManageUsers`, `CanModifyUserAuth`, `CanCreateIntegration`, `CanViewWarehouseMeteringHistory`) are consequently unused by the frontend.

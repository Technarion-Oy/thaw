...
| `UserManagementPanel.tsx` | Table of Snowflake users; reads from `ListUsers` IPC. The row context menu is minimal — **Properties** / **Enable–Disable** / **Drop** — routing to `CreateUserModal` and `UserPropertiesModal` (key-pair management lives inside the properties modal, not the menu). Gated behind `userRoleManagement` feature flag. Actions are always enabled — insufficient-privilege failures surface as an error message rather than being pre-disabled (privilege pre-checks were unreliable). |

...
| `KeyPairAuthModal.tsx` | Shared key-pair dialog. Two sources via a segmented control: **generate** a new RSA pair (`CheckAvailableKeyTools`, `GenerateKeyPair` from `internal/keypair`) or **paste** an existing public key (full PEM or stripped base64 — header/footer/whitespace removed client-side). Applies through `AlterUserProperty(username, slotProperty, key)` targeting the `slot` prop (`RSA_PUBLIC_KEY` / `RSA_PUBLIC_KEY_2`); `slotHasKey` triggers a replace-confirmation, `onApplied` reloads the parent. In create-user mode (`onKeyPicked`) it returns the key instead of applying. Exports `KeySlot` and `SLOT_PROPERTY`. |

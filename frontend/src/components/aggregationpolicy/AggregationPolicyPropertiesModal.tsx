...
export default function AggregationPolicyPropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const props = await GetObjectProperties(db, schema, "AGGREGATION POLICY", name);
      setRows(props ?? []);
    } catch (e) {
      setError(String(e));
    }
  ...
  const saveComment = async (comment: string) => {
    if (comment.trim() === "") {
      await AlterAggregationPolicy(db, schema, name, "UNSET COMMENT");
    } else {
      await AlterAggregationPolicy(db, schema, name, `SET COMMENT = ${q1(comment)}`);
    }
    await reload();
  };
  ...
  const saveBody = async () => {
    ...
    setActionError(null);
    try {
      // ALTER AGGREGATION POLICY ... SET BODY -> <expr> (the body is raw SQL, not
      // a string literal, so it is interpolated verbatim).
      await AlterAggregationPolicy(db, schema, name, `SET BODY -> ${bodyDraft}`);
  ...
  const loadReferences = async () => {
    ...
    setRefsError(null);
    try {
      const r = await GetAggregationPolicyReferences(db, schema, name);
      setRefs(r);
    } catch (e) {
  ...
  // signature / return_type are always () / AGGREGATION_CONSTRAINT, so they add
  ...
  return (
    <Modal
      ...
      title={
        <Space size={6}>
          <GroupOutlined style={{ color: "var(--link)" }} />
          <span>Aggregation Policy Properties</span>
      ...
      {rows && (
        <>
          ...
          <Text type="secondary" style={{ fontSize: 11, display: "block", marginTop: 4 }}>
            Returns <code>AGGREGATION_CONSTRAINT(MIN_GROUP_SIZE =&gt; n)</code> or{" "}
            <code>NO_AGGREGATION_CONSTRAINT()</code> — sets the minimum group size queries must aggregate to.
          </Text>
          ...
          <Text type="secondary" style={{ fontSize: 11, display: "block", marginBottom: 8 }}>
            Tables and views this policy is applied to (from ACCOUNT_USAGE — requires

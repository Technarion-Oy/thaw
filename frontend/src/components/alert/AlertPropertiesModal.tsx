...
import {
  AlertOutlined, EditOutlined, CheckOutlined, CloseOutlined,
  PauseCircleOutlined, PlayCircleOutlined, ThunderboltOutlined,
...
export default function AlertPropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const props = await GetObjectProperties(db, schema, "ALERT", name);
      setRows(props ?? []);
    } catch (e) {
      setError(String(e));
    }
  ...
  const runAction = async (clause: string, label: string) => {
    ...
    setActionError(null);
    try {
      await AlterAlert(db, schema, name, clause);
      await reload();
    } catch (e) {
  ...
  // EXECUTE ALERT is its own statement, not an ALTER ALERT clause.
  const runExecute = async () => {
    ...
    try {
      ...
      setActionError(`Execute failed: ${String(e)}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const saveComment = async (comment: string) => {
    if (comment.trim() === "") {
      await AlterAlert(db, schema, name, "UNSET COMMENT");
    } else {
      await AlterAlert(db, schema, name, `SET COMMENT = ${q1(comment)}`);
    }
    await reload();
  };
  ...
  // SHOW ALERTS reports the run state in the "state" column (e.g. "started" /
  // "suspended"); older accounts may surface it as "schedule_state".
  const state = (find("state") || find("schedule_state")).trim();
  const suspended = /suspend/i.test(state);

  // Keys handled by the editable Settings section or rendered elsewhere.
  const handledKeys = new Set(["comment", "condition", "action"]);
    ...
    >
      ...
      {rows && (
        <>
          ...
          <Space wrap>
            {state && <Tag color={suspended ? "red" : "green"}>{suspended ? "Suspended" : "Started"}</Tag>}
            ...
            <Button size="small" icon={<ThunderboltOutlined />} loading={busy} onClick={runExecute}>
              Execute now
            </Button>
          </Space>
          ...
          {condition && (
            <>
              ...
              <pre style={CODE_BLOCK}>{condition}</pre>
            </>
          )}

          {action && (
            <>
              <div style={SECTION_HEAD}>Action</div>
              <pre style={CODE_BLOCK}>{action}</pre>
            </>
          )}
        </>
      )}

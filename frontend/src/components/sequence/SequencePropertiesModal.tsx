...
import {
  NumberOutlined, EditOutlined, CheckOutlined, CloseOutlined,
...
export default function SequencePropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const props = await GetObjectProperties(db, schema, "SEQUENCE", name);
      setRows(props ?? []);
    } catch (e) {
      setError(String(e));
    }
  ...
  return (
    <Modal
      ...
      title={
        <Space size={6}>
          <NumberOutlined style={{ color: "var(--link)" }} />

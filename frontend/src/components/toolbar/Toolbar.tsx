...
export interface ToolbarProps {
  ...
  onSave: () => void;
  /** Context-specific toolbar content rendered between action buttons and session selectors. */
...
export default function Toolbar({
  ...
  return (
    ...
    >
      {/* ── Left: execution controls + action buttons ── */}
      <Space size={4}>
        {isRunning ? (
          <Button
            danger
        ...
        {/* Context-specific slot (notebook kernel status, ER design button, etc.) */}
        {contextSlot}
      </Space>

      {/* ── Center/Right: connect button or session context ── */}
      {!isConnected ? (
        <Button
          icon={<LinkOutlined />}
          type="primary"

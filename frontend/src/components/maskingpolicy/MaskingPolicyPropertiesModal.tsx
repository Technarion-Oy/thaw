...
export default function MaskingPolicyPropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const props = await GetObjectProperties(db, schema, "MASKING POLICY", name);
      setRows(props ?? []);
    } catch (e) {
      setError(String(e));
    }
  ...
  const saveComment = async (comment: string) => {
    if (comment.trim() === "") {
      await AlterMaskingPolicy(db, schema, name, "UNSET COMMENT");
    } else {
      await AlterMaskingPolicy(db, schema, name, `SET COMMENT = ${q1(comment)}`);
    }
    await reload();
  };
  ...
  const saveBody = async () => {
    ...
    setActionError(null);
    try {
      // ALTER MASKING POLICY ... SET BODY -> <expr> (the body is raw SQL, not a
      // string literal, so it is interpolated verbatim).
      await AlterMaskingPolicy(db, schema, name, `SET BODY -> ${bodyDraft}`);
  ...
  const loadReferences = async () => {
    ...
    setRefsError(null);
    try {
      const r = await GetMaskingPolicyReferences(db, schema, name);
      setRefs(r);
    } catch (e) {
  ...
  return (
    <Modal
      ...
      title={
        <Space size={6}>
          <EyeInvisibleOutlined style={{ color: "var(--link)" }} />
          <span>Masking Policy Properties</span>

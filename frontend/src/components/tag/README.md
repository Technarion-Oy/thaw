...
- `TagReferencesModal` is opened from the **Tag References…** item on every
  object's context menu in `components/layout/Sidebar.tsx` (and on **column**
  nodes, which pass the `column` prop for single-column mode); it delegates to

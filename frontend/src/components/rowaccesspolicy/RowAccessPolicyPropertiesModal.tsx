...
export default function RowAccessPolicyPropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const props = await GetObjectProperties(db, schema, "ROW ACCESS POLICY", name);
      setRows(props ?? []);
    } catch (e) {
      setError(String(e));
    }
  ...
  const saveComment = async (comment: string) => {
    if (comment.trim() === "") {
      await AlterRowAccessPolicy(db, schema, name, "UNSET COMMENT");
    } else {
      await AlterRowAccessPolicy(db, schema, name, `SET COMMENT = ${q1(comment)}`);
    }
    await reload();
  };
  ...
  const saveBody = async () => {
    ...
    setActionError(null);
    try {
      // ALTER ROW ACCESS POLICY ... SET BODY -> <expr> (the body is raw SQL, not
      // a string literal, so it is interpolated verbatim).
      await AlterRowAccessPolicy(db, schema, name, `SET BODY -> ${bodyDraft}`);
  ...
  const loadReferences = async () => {
    ...
    setRefsError(null);
    try {
      const r = await GetRowAccessPolicyReferences(db, schema, name);
      setRefs(r);
    } catch (e) {
  ...
  return (
    <Modal
      ...
      title={
        <Space size={6}>
          <SafetyOutlined style={{ color: "var(--link)" }} />
          <span>Row Access Policy Properties</span>

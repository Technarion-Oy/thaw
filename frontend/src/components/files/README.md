...
- **IPC**: `ListDirectory`, `ReadFile`, `SearchFiles`, `RevealInFinder`, `DeleteFile`, `DeleteDirectory`, `RenameFile`, `CopyFile`, `CreateDirectory`, `CreateFile`, `DuplicateFile`, `StartFileWatcher`, `StopFileWatcher`, `GetPlatformOS` — all from `wailsjs/go/app/App`.

...
export default function ProcedurePropertiesModal({ db, schema, name, args, onClose }: Props) {
  ...
  const saveComment = async (comment: string) => {
    if (comment.trim() === "") {
      await AlterProcedure(db, schema, name, args, "UNSET COMMENT");
    } else {
      await AlterProcedure(db, schema, name, args, `SET COMMENT = ${q1(comment)}`);
    }
    await reload();
  };
  ...
  const toggleSecure = async (next: boolean) => {
    ...
    setActionError(null);
    try {
      await AlterProcedure(db, schema, name, args, next ? "SET SECURE" : "UNSET SECURE");
      await reload();
    } catch (e) {
  ...
  const handledKeys = new Set(["comment", "is_secure"]);
  ...
  return (
    <Modal
      ...
      title={
        <Space size={6}>
          <CodeOutlined style={{ color: "var(--link)" }} />

...
- The **Tags** section is the shared `TagsRow` + `useObjectTags` hook (see
  `../shared/README.md`): current tags are read via `GetObjectTagReferences`
  (`INFORMATION_SCHEMA.TAG_REFERENCES`, object domain `CORTEX SEARCH SERVICE`) and
  add/remove issue `SET`/`UNSET TAG` through `AlterCortexSearchService`; a read
  error degrades to "no tags" without blocking edits.

...
Renders an interactive ER diagram using `@xyflow/react` from Snowflake table/column/FK metadata fetched by the backend. Supports schema filtering, drag-to-rearrange, zoom, pan, minimap, and clipboard copy of Mermaid source. Also hosts `ERDesigner`, a side-by-side modal for visually defining new tables (**Add Table** opens the full `CreateTableModal` from `database/`, then places the result on the canvas; inline editing, FK connection drawing, and generated DDL execution follow).
...
| `ERDesigner.tsx` | Interactive table designer. Left sidebar with table/column CRUD forms, right panel with `ERCanvas` in edit mode. **Add Table** opens `database/CreateTableModal` in its return-definition mode (`onDefine`) for the full create feature set (table type, clustering, retention, change-tracking, comments, and the ƒ DEFAULT picker on every column); the returned `TableConfig` is mapped to a `DesignerTable` (columns + a `DesignerTable.options` blob) and placed on the canvas, where schema and FKs are then wired. Generates diff-based SQL (`CREATE TABLE`, `ALTER TABLE`, `DROP TABLE`) and executes via `ExecuteQuery`. A new table's `options` flow into its `CREATE TABLE` via `shared/tableDdl.ts` (`createTableClause`/`tableOptionsClauses`); tables loaded from Snowflake or lacking options keep the historical `CREATE TABLE IF NOT EXISTS`. Syncs state to backend MCP cache (mount/unmount/debounced changes) and listens for `mcp:modify-er-designer` events. The per-column DEFAULT field is shown for **new columns only** (`isNewColumn(t, c)`): the free-text `Input` for any new column (a `CREATE TABLE` column or an `ADD COLUMN` on an existing table, both accept a literal), plus the `DefaultFunctionPicker` ƒ shortcut **only for new tables** (`!isExistingTable(t)`) since Snowflake rejects function-expression defaults on `ALTER … ADD COLUMN`. Existing columns' defaults aren't editable/diffed (`ALTER COLUMN … SET DEFAULT` is heavily restricted). Emission goes through `shared/columnDdl.ts` `columnConstraints`, so `DEFAULT` precedes `NOT NULL` per Snowflake grammar. `initFromERData` strips IDENTITY/AUTOINCREMENT generator clauses from `COLUMN_DEFAULT` (`literalDefault`) so duplicating an identity column doesn't emit an invalid literal `DEFAULT`. Baseline/new detection uses the shared uppercase `baselineTableKey` from `erTypes.ts` (case-insensitive, matching `generateDiffSQL`). The AI-merge path (`mergeAITablesIntoDesigner`) preserves a user-set default via the AI's echoed `defaultValue` (round-tripped through MCP `ERDesignerColumnOut`/`erDesignerColumnIn`), falling back to a same-name match. |

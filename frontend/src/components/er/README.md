...
| `ERDesigner.tsx` | Interactive table designer. Left sidebar with table/column CRUD forms, right panel with `ERCanvas` in edit mode. Generates diff-based SQL (`CREATE TABLE`, `ALTER TABLE`, `DROP TABLE`) and executes via `ExecuteQuery`. Syncs state to backend MCP cache (mount/unmount/debounced changes) and listens for `mcp:modify-er-designer` events. The per-column DEFAULT field (free-text `Input` + `DefaultFunctionPicker` ƒ shortcut from `shared/`) is a **new-table-only** control, shown only when `!isExistingTable(t)` — Snowflake rejects function-expression defaults on `ALTER … ADD COLUMN`, and literal defaults on existing tables belong in the dedicated Add Column dialog. New-table columns emit `DEFAULT <expr>` (before `NOT NULL`, per Snowflake grammar) in the generated `CREATE TABLE`. Existing columns' default changes are not diffed (Snowflake restricts `ALTER COLUMN … SET DEFAULT`). The AI-merge path (`mergeAITablesIntoDesigner`) preserves a user-set default by matching column name so an assistant edit doesn't wipe it; `defaultValue` is also surfaced read-only to MCP via `ERDesignerColumnOut`. |

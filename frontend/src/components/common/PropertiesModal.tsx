...
interface Props {
  title: string;
  rows: app.PropertyPair[] | null;  // null = loading
  error: string | null;
  onClose: () => void;
...
function ColumnCommentsSection({ db, schema, table }: TableContext) {
  const [colComments, setColComments] = useState<app.ColumnComment[] | null>(null);
...
type PropKey = keyof app.TableSettings;
...
function TableSettingsSection({ db, schema, table }: { db: string; schema: string; table: string }) {
  const [settings, setSettings]   = useState<app.TableSettings | null>(null);
  const [loadError, setLoadError] = useState<string | null>(null);

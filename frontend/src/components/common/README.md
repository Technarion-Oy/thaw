...
| `ObjectParametersModal.tsx` | Editable, searchable view of **all** parameters for a single object, self-loading via `GetObjectParameters(objectType, nameParts)`. Props: `objectType` (DATABASE/SCHEMA/TABLE/WAREHOUSE/TASK/USER), `nameParts` (identifier parts, e.g. `[db]`, `[db, schema]`, `[wh]`), `title` (header label), `readOnly` (renders values without edit/reset/toggle controls — used for `INFORMATION_SCHEMA`, issue #854). Booleans render as staged `ConfirmSwitch` toggles, other params as inline text inputs; a `level` column highlights values set at **this** object (green tag) vs inherited from a higher scope (muted tag). Saves call `SetObjectParameter` (`ALTER … SET`); a reset button on overridden rows calls `UnsetObjectParameter` (`ALTER … UNSET`, revert to inherited). Edits require ownership/privilege — Snowflake's privilege error surfaces via `message.error`, and unprivileged roles seeing empty rows get a graceful "no parameters visible" state. Opened from the "Parameters…" footer button in `DatabasePropertiesModal`, `SchemaPropertiesModal`, and `WarehousePropertiesModal` (issue #813). |

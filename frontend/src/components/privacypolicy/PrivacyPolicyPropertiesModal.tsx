...
import {
  GetObjectProperties, AlterPrivacyPolicy, GetPrivacyPolicyReferences,
...
export default function PrivacyPolicyPropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const objTags = useObjectTags({
    kind: "PRIVACY POLICY", db, schema, name,
    alter: (clause) => AlterPrivacyPolicy(db, schema, name, clause),
  });

...
import {
  SecurityScanOutlined, EditOutlined, CheckOutlined, CloseOutlined, ReloadOutlined,
  PlusOutlined,
} from "@ant-design/icons";
import {
  GetObjectProperties, AlterPrivacyPolicy, GetPrivacyPolicyReferences, GetPrivacyPolicyTags,
...
export default function PrivacyPolicyPropertiesModal({ db, schema, name, onClose }: Props) {
  ...
  const reloadTags = useCallback(async () => {
    try {
      const t = await GetPrivacyPolicyTags(db, schema, name);
      setTags(t);
    } catch {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const props = await GetObjectProperties(db, schema, "PRIVACY POLICY", name);
      setRows(props ?? []);
    } catch (e) {
      setError(String(e));
    }
  ...
  const saveComment = async (comment: string) => {
    if (comment.trim() === "") {
      await AlterPrivacyPolicy(db, schema, name, "UNSET COMMENT");
    } else {
      await AlterPrivacyPolicy(db, schema, name, `SET COMMENT = ${q1(comment)}`);
    }
    await reload();
  };
  ...
  const saveBody = async () => {
    ...
    setActionError(null);
    try {
      // ALTER PRIVACY POLICY ... SET BODY -> <expr> (the body is raw SQL, not a
      // string literal, so it is interpolated verbatim).
      await AlterPrivacyPolicy(db, schema, name, `SET BODY -> ${bodyDraft}`);
  ...
  const addTag = async () => {
    ...
    try {
      ...
      await AlterPrivacyPolicy(db, schema, name, `SET TAG ${tn} = ${q1(tagValue)}`);
      setAddingTag(false);
      setTagName("");
      setTagValue("");
      await reloadTags();
  ...
  const removeTag = async (qualifiedName: string) => {
    setActionError(null);
    try {
      await AlterPrivacyPolicy(db, schema, name, `UNSET TAG ${qualifiedName}`);
      await reloadTags();
    } catch (e) {
  ...
  const loadReferences = async () => {
    ...
    setRefsError(null);
    try {
      const r = await GetPrivacyPolicyReferences(db, schema, name);
      setRefs(r);
    } catch (e) {
  ...
  // Resolve the tag columns once (GetPrivacyPolicyTags returns INFORMATION_SCHEMA
  ...
  return (
    <Modal
      ...
      title={
        <Space size={6}>
          <SecurityScanOutlined style={{ color: "var(--link)" }} />
          <span>Privacy Policy Properties</span>
      ...
      {rows && (
        <>
          ...
          <Text type="secondary" style={{ fontSize: 11, display: "block", marginBottom: 8 }}>
            Tables and views this policy is applied to (from ACCOUNT_USAGE —

...
- Applying a backup policy on **create** is handled by passing the policy name to `CreateBackupSet` (the backend applies it with the matching name reference) — not a separate `AlterBackupSet` call. Policy names emitted into `APPLY BACKUP POLICY` and free-text values (`SET COMMENT`, `SET SCHEDULE`) go through `quoteIdent` / the shared backslash-safe `escapeTextLit` (`../shared/sqlEscape`), mirroring the backend. On the create+apply-policy path, `handleCreate` refreshes the row list even on error, since the set may already exist when the follow-up policy `ALTER` fails.

...
| `CrossTabSearch.tsx` | Search/replace panel triggered by `⌘⇧H` / `Ctrl+Shift+H`. Searches all tabs (SQL, YAML, Python) and notebook cell sources. Navigates via `thaw:scroll-to-line` / `thaw:editor-ready` events. Supports regex with back-references, case-sensitive toggle, and match counter. Result rows label their tab with `tabDisplayTitle` (`tabTitle.ts`), so an unnamed scratch tab is named the same way the strip names it. Always available — the `crossTabSearch` feature flag was removed in issue #567. |
| `CrossTabSearch.test.ts` | Unit tests for `getNotebookCellSources` and related helpers in `CrossTabSearch.tsx`. |
| `TabBar.tsx` | Renders the tab strip above the editor. Supports drag-to-reorder, right-click context menu (close, close others, split, duplicate), bulk-close confirmation, and split-tab mode. MCP-created tabs (`tab.mcpOrigin`) display a `RobotOutlined` icon in the accent color — this check takes priority over the notebook `ExperimentOutlined` icon, so MCP-created notebooks also show the robot badge. The right-click menu is a per-tab AntD `Dropdown` (`trigger={["contextMenu"]}`, built by `buildTabMenuItems`) — same idiom as the query-history context menu in `QueryPage.tsx` — with icons, `danger: true` on Close Others/Close All, an `extra` keybinding hint on Close (`⌘W`), and "Split with" as a native submenu (`children`) instead of a hand-rolled hover panel. A far-right `CaretDownOutlined` button (after the `+` new-scratch button) opens the **Active Files** dropdown: a searchable list of every open tab (icon + dirty `•`/orphan `↺` prefix + title), styled with the same `.ctx-item` class (rounded, inset rows) so it reads as one visual language with the tab context menu; clicking an entry calls `activateTab`. Each row also has a hover-revealed close button (`.ctx-item-close`, routed through the same `thaw:request-close-tab` flow so dirty tabs still prompt) and its own right-click context menu — the same `buildTabMenuItems` used by the strip, so tabs that overflow the strip can be closed/renamed/split without scrolling (issue #767). Rename from a panel row (`startRenameFromPanel`) closes the panel and `scrollIntoView`s the tab (tracked via `tabRefs`) so the strip's inline rename input is visible; the panel's outside-click dismiss handler ignores clicks inside an `.ant-dropdown` portal so a context-menu action isn't swallowed by the panel unmounting. **Truncation tooltips (#829):** long titles that overflow reveal the full title/path on hover, measured on hover (`scrollWidth > clientWidth`) so short names that fit get no redundant tooltip. Panel rows use the `OverflowTooltip` helper (AntD `Tooltip` with `overlayStyle={{ zIndex: 10000 }}` to clear the `position:fixed` panel, same as its context menu); the strip folds the full label into each tab's existing session-ID tooltip (set via `truncatedTabId`) rather than nesting a second tooltip. The label is `tabFullLabel(t)` — the backing file's full path for file tabs (so several files from one directory are distinguishable), otherwise the title. Also toggled by `⌘⇧E` / `Ctrl+Shift+E` via the `thaw:open-active-files` window event; closes on outside click or Escape. Non-file tabs (no backing path, not a diff) can be **renamed** — double-click the title or use the context-menu "Rename" — via `queryStore.renameTab`; file tabs keep the filename-derived title. **Preview tabs (#849):** a `tab.preview` tab renders its title in *italic* and is promoted to permanent on double-click (the tab div's `onDoubleClick` calls `queryStore.promoteTab`; the title span lets file-tab double-clicks bubble to it, only consuming the event to rename non-file tabs) — matching the file browser's single-click-preview / double-click-pin flow. The `preview` flag is part of `tabStripSignature` so promotion (which flips it once) repaints the strip. The Active Files panel rows italicize preview tabs too. A preview closes as soon as the user navigates away from it (`dropStalePreview` in `queryStore`, #889), so the strip never accumulates italic peek tabs; one that holds work (dirty, running, orphaned, or the split target) turns permanent instead. New scratch tabs are numbered `SQL n` (`nextScratchTitle` in `queryStore`). Reads/writes `queryStore`. **Strip polish (#881)** — no behavior changes, all six pieces aimed at "five open queries should look like five different things": (1) **derived titles** — a scratch tab still on its auto-generated title renders `tabDisplayTitle` (see `tabTitle.ts`) instead, e.g. `SELECT · ORDERS`; `Tab.title` is untouched, so rename/save/numbering are unaffected. The panel rows, the "Split with" submenu, the Active Files *filter*, and the truncation tooltip all use the same derived title, so nothing matches a name the user can't see. (2) **No icon on plain scratch tabs** — `tabIcon` returns null for them, so the file/diff/notebook/MCP glyphs become a signal instead of decoration; panel rows keep a fixed-width `.qtab-panel-icon` slot so titles stay aligned. (3) **Dirty `•` / orphan `↺` moved into the reserved 16 px close slot** (`tabMark`, VS Code convention) — going dirty no longer shifts the title sideways, and the marker swaps to ✕ on hover *in CSS*. Panel rows keep the marker as a leading prefix (`tabPrefix`), where a full-width list has no shift to avoid. (4) **Inset hairline separators** instead of a full-height `borderRight` per tab, dropped around the active/hovered tab. (5) **Overflow is visible** — the strip tracks its clipped edges (scroll + `ResizeObserver` + tab-set changes) and renders an edge fade for whichever side still has tabs, plus a tab-count badge on the Active Files caret while clipped. **Auto-reveal (#889):** an effect keyed on `[activeTabId, activePath]` calls `scrollIntoView({ inline: "nearest", block: "nearest" })` on the active tab's node (from `tabRefs`), so every activation path that only touches store state — opening a file from the file explorer (preview *or* pinned, new tab or re-activated), picking a tab from the Active Files panel, `+` while the strip is overflowed, ⌘W activating an off-screen neighbour, the preview-tab recycle in `openFile` (same tab id, new path — which is why `path` is a dependency) — ends with the tab visible instead of hidden behind an edge fade. `inline: "nearest"` makes it a no-op for an already-visible tab, so clicking a visible tab never shifts the strip; keying on `tabsSig` instead of the active id/path would yank the scroll position whenever a *background* tab's metadata changed. (6) **Real tab semantics** — `role="tablist"` (holding only tabs; the `+` button is a sibling) with `role="tab"`/`aria-selected`, roving `tabIndex`, and ←/→/Home/End navigation with automatic activation; `+` and the caret are real `<button>`s. **Perf (#762, #881):** the strip must not re-render on every keystroke (a `setSql` rebuilds `tabs`), so `TabBar` subscribes to a `tabsSig` signature — `s.tabs.map(tabStripSignature).join(…)` — of only the fields it renders (id/title/**derived title**/path/kind/diff/mcpOrigin/orphaned/dirty/preview) and snapshots the live array via `useMemo(() => useQueryStore.getState().tabs, [tabsSig])` — display-correct because every rendered field is in the signature. The exported `tabStripSignature(tab)` helper is the single source of that field list (NUL-separated so space-bearing titles/paths can't alias) and is unit-tested in `TabBar.test.ts` so adding a rendered field without adding it to the signature fails the test. The derived title is the only signature field that reads `sql`; it changes a handful of times while the first statement is written and then goes quiet, and `hoveredId` state is gone entirely — hover is `:hover` in `global.css`, so pointer movement across the strip costs zero renders. |
| `tabTitle.ts` | Provisional tab titles (#881). `provisionalTitle(sql)` labels a SQL buffer from its **first statement** — `SELECT · ORDERS`, `CREATE TABLE · CUSTOMERS`, `SHOW WAREHOUSES` — returning null when there is nothing to describe (empty or comments-only). `tabDisplayTitle(tab)` is what the UI renders: the derived title only for a tab that still carries its auto-generated `SQL n` (`isDefaultTitle`), has no backing path and is SQL-kind; everything else keeps `Tab.title`. Object kinds for `CREATE`/`ALTER`/`DROP` come from the generated `OBJECT_KINDS` registry (longest-first alternation, so `DYNAMIC TABLE` beats `TABLE`) plus a short list of account/session kinds the object browser doesn't carry. **Deliberately a regex heuristic, not the real grammar in `internal/sqleditor`:** it runs inside `tabStripSignature` on the store's hot path, so IPC or a real parse per keystroke is out. Known simplifications: comment delimiters are stripped even inside string literals, and only the first 2 000 characters are scanned. |
| `tabTitle.test.ts` | Unit tests for `provisionalTitle` (one case per statement family, quoted-identifier case preservation, first-statement-only, leading comments, long-name truncation, unknown-keyword fallback, no-throw on malformed input) and `tabDisplayTitle` (derives only for unnamed scratch tabs; file-backed and non-SQL tabs untouched). |
| `EditorPreferencesModal.tsx` | Modal for editor preferences (keyword case, indent style/size, font, font size). Calls `GetEditorPrefs`/`SaveEditorPrefs` IPC. Shows a live SQL preview using `formatSQL`. Also hosts the frontend-only **"Single-click opens a preview tab"** toggle (#849) — staged in local state (so Cancel discards it) and committed to `editorTabPrefsStore.setPreviewTabsEnabled` in `handleSave` alongside the backend prefs. |
...
keystroke, and each call can fire 2–3 Wails round-trips (JOIN ON resolution via
...
Its refs are scoped to the **statement at the cursor** (`statementTextAtLine`), not to the text
before the cursor like the JOIN-ON shortcut's: in a multi-statement worksheet `prefixFull` drags
...
context db + schema, 2-part → context db + written schema, 3-part → all three written parts
(#918). Without that scoping any bare name colliding with something the sidebar happened to have
...
`useCtx` comes from `useContextAt(pos)` — `GetUseContextAt` memoized per (model version, line) —
so a worksheet that opens with `USE SCHEMA X` links its bare names before it has ever been run,
matching the precedence diagnostics already apply. The TABLE/VIEW preference on a
within-namespace collision, the excluded callable kinds, and the untracked `SEARCH_PATH` ceiling
are documented on the Go function.

`editor.onMouseMove` uses it via the shared
...
`does not exist or not authorized` means the store entry is stale — that object is evicted via
`objectStore.removeObject(db, schema, name, kind)` (kind included, so a same-named STREAM and
TABLE in one schema are evicted independently) and the underline cleared, so the doomed
`GET_DDL` is never re-fired. Snowflake does not distinguish the two halves of that error, so a
listed object the current role has merely lost access to is evicted the same way as a dropped
one; re-expanding the schema in the sidebar re-adds whatever is still visible. `cmdModHeld` tracks the modifier
...
- **Cross-tab search panel**: the panel is conditionally rendered by `QueryPage` on the
  `crossTabSearchOpen` state (no feature flag — always available since #567); its state (search

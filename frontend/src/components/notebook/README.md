...
- **Clipboard**: a module-level `installCopyHandler` patches `Cmd+C` on each cell container to route clipboard writes through `ClipboardSetText` (WKWebView blocks native clipboard API).

...
| Query tab strip | `.qtab-scroll` / `.qtab-list` (scroll region + the `role="tablist"` that holds only tabs), `.qtab` / `.qtab-active` / `.qtab-marked` (a tab and its states, including the inset `::before` hairline separator that is dropped around the active and hovered tabs), `.qtab-title` / `.qtab-preview` / `.qtab-rename`, `.qtab-slot` with `.qtab-mark*` / `.qtab-close` (the one reserved 16 px slot holding the dirty/orphan marker at rest and the ✕ on hover), `.qtab-fade-left` / `.qtab-fade-right` (overflow cues), `.qtab-btn` / `.qtab-count` (the `+` and Active Files buttons), `.qtab-panel-icon`. Hover lives here rather than in `TabBar.tsx` on purpose: it used to be React state, which re-rendered the strip on every pointer move (#881) |

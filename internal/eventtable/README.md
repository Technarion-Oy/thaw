...
layout**, the statement carries no column list — but it does support `CLUSTER BY`
on the predefined columns plus the supported table-level properties:
...
- `EventTableConfig` — name + case sensitivity, `OrReplace`/`IfNotExists`
  (mutually exclusive), `ClusterBy`, `DataRetentionTimeInDays`,
  `MaxDataExtensionTimeInDays`, `ChangeTracking`, `DefaultDdlCollation`,
  `CopyGrants`, `Comment`, `Tags`.
...
- `OR REPLACE` and `IF NOT EXISTS` are mutually exclusive; when both are set the
  ...
  flags the same conflict, plus column definitions and `TRANSIENT` (neither of
  which this builder ever emits). `CLUSTER BY` is **valid** for event tables and
  is not flagged.

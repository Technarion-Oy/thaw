...
- `save()` writes what `buildDiskConfig` returns: each secret is blanked on disk **only once it is safely in the store**. The store is **never overwritten** from a value read out of `config.json` (`stored()` writes only when the store lacks the key), so a stale/synced/restored `config.json` can't clobber a newer secret already in the OS store. During the `Load()` migration path a legacy secret that can't be stored is left on disk (`0600`) rather than lost, and retried next load.

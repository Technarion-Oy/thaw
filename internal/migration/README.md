...
- The `blue_green_swap` strategy rewrites the local DDL table name via `replaceDDLTableName`, creates the temp table, copies shared columns, swaps with `ALTER TABLE … SWAP WITH`, then drops the temp. `replaceDDLTableName` locates the `TABLE` keyword *token* and splices the identifier path that follows it by byte offset, so a quoted name containing the word (`CREATE TABLE "ORDERS TABLE" (…)`) or a preceding comment cannot shift the replacement.
...
- `replaceDDLTableName` rewrites the identifier after the **first** `TABLE` keyword token and returns the DDL unchanged when there is none. It no longer requires a column-list `(` to be present, so CTAS and bare `CREATE TABLE t` headers are rewritten too — previously they were left with their original name, which would have collided with the real table.

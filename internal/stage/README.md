...
| `UploadDirToStage(ctx, client, localDir, stageName, overwrite)` | one `PUT` per file (via `UploadFileToStage`, `AUTO_COMPRESS=FALSE`) | Recursively uploads a local folder, preserving subdirectories under `@stage/<reldir>`; skips `.git/`, `__pycache__/`, virtualenv/dependency trees (`venv/`, `env/`, `node_modules/`), other hidden files/dirs, and `.DS_Store` — but **keeps** `.streamlit/` (config.toml theming). The walk/grouping is the pure, unit-tested `planDirUploads(root)`. Used by `streamlit.DeployStreamlit`. |

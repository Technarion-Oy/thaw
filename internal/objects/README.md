...
| Function | Emits |
|---|---|
| `BuildObjectPropertiesQuery(db, schema, kind, name)` | `SHOW <OBJECTS> LIKE '<name>' IN [SCHEMA/DATABASE] <fqn>` — supports 29 object kinds |
...
`GetObjectProperties` is the single entry point for the Properties side-panel. It handles all 37 supported object kinds (DATABASE, SCHEMA, TABLE, VIEW, DYNAMIC TABLE, EXTERNAL TABLE, ICEBERG TABLE, HYBRID TABLE, EVENT TABLE, MATERIALIZED VIEW, ALERT, TAG, MASKING POLICY, ROW ACCESS POLICY, PASSWORD POLICY, SESSION POLICY, AGGREGATION POLICY, NETWORK RULE, IMAGE REPOSITORY, SERVICE, STREAMLIT, FUNCTION, EXTERNAL FUNCTION, DATA METRIC FUNCTION, PROCEDURE, SEQUENCE, STAGE, STREAM, TASK, FILE FORMAT, PIPE, SECRET, GIT REPOSITORY, DBT PROJECT, WAREHOUSE, ROLE, USER) through a `switch` on `kind`. STAGE additionally merges `DESCRIBE STAGE` rows (keyed as `parent.property`), MASKING POLICY, ROW ACCESS POLICY, and AGGREGATION POLICY merge the corresponding `DESCRIBE` signature / return type / body, NETWORK RULE merges the `DESCRIBE NETWORK RULE` value_list, SERVICE merges the `DESCRIBE SERVICE` spec / dns_name, and STREAMLIT merges the `DESCRIBE STREAMLIT` root_location / main_file, to expose configuration the corresponding `SHOW` omits.

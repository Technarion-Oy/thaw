...
- `MkDir(path, allowedRoot)` creates the target **and any missing parents**, but rejects a target that already exists — `MkDir`'s equivalent of `WriteFileInRoot`'s `O_EXCL`. Bare `os.MkdirAll` succeeds silently on an existing directory, which left the caller unable to tell "created" from "was already there": the file browser's inline folder create reported success and inserted a duplicate tree node for a folder it hadn't created. The target is always created with **`os.Mkdir`**, never an `Lstat`-then-`MkdirAll` pair — that check-then-act is not equivalent to `O_EXCL`, and **"Open Folder in New Window…" makes the race reachable**: two processes on one export directory can both pass the check and both report success. Only intermediate parents go through `MkdirAll`, whose idempotence is the point of "and any necessary parents". `TestMkDir_ConcurrentSameName` and `TestMkDir_ConcurrentSameNameNested` race eight goroutines down each branch — parent present, and parent missing so the `ENOENT` → `MkdirAll(parent)` → retry path runs — and assert exactly one wins. The nested one is what pins the retry using `os.Mkdir` on the final component rather than `MkdirAll`: with `MkdirAll` there, every racer succeeds. The sole caller (`App.CreateDirectory` → the file browser's inline create) **depends on being told when it created nothing** — a future caller that just wants "make sure this path exists" should call `os.MkdirAll` itself rather than relaxing `MkDir` back to idempotent.

...
// AlterProjectionPolicy runs an ALTER PROJECTION POLICY statement for the given
...
func (a *App) AlterProjectionPolicy(database, schema, name, clause string) error {
	return a.alterObject("PROJECTION POLICY", database, schema, name, clause)
}

// GetProjectionPolicyReferences returns the columns to which the given
// projection policy is currently applied, by querying
// SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES filtered to POLICY_KIND =
// 'PROJECTION_POLICY'. The view requires governance privileges (e.g. the

...
| `database.go` | `AlterDatabase` (free-form `ALTER DATABASE "<db>" <clause>` — **one-level**, so simpler than `AlterSchema`; drives every SET/UNSET plus RENAME TO, SWAP WITH, SET/UNSET TAG, and the ENABLE/DISABLE REPLICATION/FAILOVER, REFRESH, PRIMARY verbs from the `DatabasePropertiesModal`) and `GetDatabaseParameters` (`SHOW PARAMETERS IN DATABASE`, the fallback source for the parameters SHOW DATABASES omits). |

...
| `eventtable.go` | `AlterEventTable` (free-form `ALTER TABLE … <clause>` for SET/UNSET COMMENT / SET CHANGE_TRACKING / RENAME TO — event tables share the standard TABLE grammar and have no dedicated ALTER/DROP EVENT TABLE statement); CREATE builder in `builders.go` delegates to `internal/eventtable`. |

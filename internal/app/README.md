...
| `users.go` | User/role management IPC: `ListUsers`, `GetUserDDL`, `AlterUserProperty` (delegates to `internal/users`), plus key generation `CheckAvailableKeyTools` / `GenerateKeyPair` (delegates to `internal/keypair`). RSA public keys are registered through `AlterUserProperty(name, "rsaPublicKey"\|"rsaPublicKey2", value)` — there is no separate set-public-key IPC. |

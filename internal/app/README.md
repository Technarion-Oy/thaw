...
| `objects.go` | `ListDatabases`, `ListSchemas`, `ListUserSchemas` (schemas minus the read-only `INFORMATION_SCHEMA`, for object/DDL/governance target pickers), `ListObjects`, `ListBasicObjects`, `ClearObjectCache`, `ClearObjectCacheForDatabase`, `DropDatabase`, `DropSchema`, `GetObjectDDL`, and related object-management methods. |

...
| File | Contents |
|------|----------|
| `app.go` | `App` struct definition, `NewApp`, `startup`/`shutdown` lifecycle, `Connect`/`CancelConnect`/`Disconnect`/`IsConnected`, tab-session management (`getOrInitTabSession`, `InitTabSession`, `CloseTabSession`, `evictIfNeeded`, `evictIdleSessions`, `runIdleEvictionLoop`, `applySessionConfig`), `GetAppInfo`. Reads the `--workdir=<dir>` launch arg into `workdirOverride` (set by "Open Folder in New Window"): `startup` roots this instance there and retitles the window; `GetGitConfig`/`SaveGitConfig` (in `git.go`) return/pin `ExportDir` to the override so an override window never persists its folder to the shared config. |

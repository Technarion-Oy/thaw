...
| `streamlit.go` | `AlterStreamlit` (free-form `ALTER STREAMLIT … <clause>` for RENAME TO and SET/UNSET of MAIN_FILE/QUERY_WAREHOUSE/TITLE/COMMENT/EXTERNAL_ACCESS_INTEGRATIONS); CREATE builder in `builders.go` delegates to `internal/streamlit`. Local-app deploy: `DeployStreamlit` (upload a local folder → temp stage → `CREATE STREAMLIT`, delegates to `streamlit.DeployStreamlit`, which reuses `stage.UploadDirToStage` + `BuildCreateStreamlitSql`) and `DetectStreamlitMainFile` (pick the entrypoint / list `*.py` candidates for the deploy modal, via `internal/streamlit`). PUT is always available, so the deploy is not feature-gated. Templates: `ListStreamlitTemplates` and `CreateStreamlitFromTemplate(templateName, destDir)` scaffold a new local app from `Snowflake-Labs/snowflake-demo-streamlit` (via `internal/streamlittemplate`); neither needs a Snowflake connection. |

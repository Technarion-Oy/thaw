...
| `objects.go` | `ListDatabases`, `ListUserDatabases` (databases minus shared/imported ones, for DDL/governance target pickers like SWAP WITH), `ListSchemas`, `ListUserSchemas` (schemas minus the read-only `INFORMATION_SCHEMA`, for object/DDL/governance target pickers), `ListObjects`, `ListBasicObjects`, `ClearObjectCache`, `ClearObjectCacheForDatabase`, `DropDatabase`, `DropSchema`, `GetObjectDDL`, `GetObjectDependencies` (DDL-parsed depends-on tree), `GetObjectUsageDependencies` (ACCOUNT_USAGE.OBJECT_DEPENDENCIES edges in either direction — the unified Dependencies & References modal), and related object-management methods. |

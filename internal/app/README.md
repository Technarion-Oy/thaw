...
| `agent.go` | `AlterAgent` (free-form `ALTER AGENT … <clause>` covering the full grammar: `SET COMMENT`, `SET PROFILE`, `MODIFY LIVE VERSION SET SPECIFICATION = $$…$$`; agents have no RENAME/UNSET/TAG) and `DescribeAgent` (`DESCRIBE AGENT` → the `agent_spec` column the live-spec editor reads, which `SHOW AGENTS` omits); CREATE builder in `builders.go` delegates to `internal/agent`. GET_DDL works via the `CORTEX_AGENT` object type. |

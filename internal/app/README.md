...
| `externaltable.go` | `AlterExternalTable` (free-form `ALTER EXTERNAL TABLE … <clause>` for REFRESH/SET AUTO_REFRESH/SET/UNSET); CREATE builder in `builders.go` delegates to `internal/externaltable`. |

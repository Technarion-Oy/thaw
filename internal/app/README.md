...
| `parameters.go` | Generic object-level parameter viewer/editor IPC: `GetObjectParameters` (`SHOW PARAMETERS IN <TYPE> <name>`), `SetObjectParameter` (`ALTER <TYPE> <name> SET <p> = <v>`), `UnsetObjectParameter` (`ALTER … UNSET <p>` → revert to inherited). `objectType` is validated against an allowlist (DATABASE/SCHEMA/WAREHOUSE/TABLE/TASK/USER) and the parameter name against a bare-identifier pattern before interpolation; `nameParts` are quoted per part. Editing needs ownership or the relevant privilege — Snowflake's privilege error surfaces inline. Backs `ObjectParametersModal`, opened from the object properties modals (issue #813). |
...
| `session.go` | `GetSessionContext`, `GetTabSessionID`, `GetQuotedIdentifiersIgnoreCase`, `UseRole`/`UseWarehouse`/`UseDatabase`/`UseSchema`, session-parameter getters/setters, `GetAccountParameters` / `SetAccountParameter` (`SHOW PARAMETERS IN ACCOUNT` + `ALTER ACCOUNT SET`; editing requires ACCOUNTADMIN), `GetClientVersionInfo` (general `SELECT SYSTEM$CLIENT_VERSION_INFO()` → supported/recommended client & driver versions, reusable by any feature). |
| `objects.go` | `ListDatabases`, `ListUserDatabases` (databases minus shared/imported ones, for DDL/governance target pickers like SWAP WITH), `ListSchemas`, `ListUserSchemas` (schemas minus the read-only `INFORMATION_SCHEMA`, for object/DDL/governance target pickers), `ListObjects`, `ListBasicObjects`, `SearchAccountObjects` (account-wide object search for the sidebar search box — one `SHOW … IN ACCOUNT` per kind, optional LIKE pushdown), `ClearObjectCache`, `ClearObjectCacheForDatabase`, `DropDatabase`, `DropSchema`, `GetObjectDDL`, `GetObjectDependencies` (DDL-parsed depends-on tree), `GetObjectUsageDependencies` (ACCOUNT_USAGE.OBJECT_DEPENDENCIES edges in either direction — the unified Dependencies & References modal), and related object-management methods. |
| `warehouse.go` | Warehouse IPC: `GetWarehouseDDL`, `AlterWarehouseProperty`/`Suspend`/`Resume`/`AbortAllQueries`/`Rename`, `AlterWarehouse` (free-form `ALTER WAREHOUSE <name> <clause>` — backs the object-level Tags section of the warehouse Properties modal; account-level, so the name is a bare double-quoted identifier), `GetWarehouseParameters`, `GetWarehouseMeteringHistory`. Delegates to `internal/warehouse`. |
| `table.go` | Table settings queries and column DDL builders; delegates to `internal/table` and `internal/column`. Also `AlterTable` (free-form `ALTER TABLE <fqn> <clause>`) — backs the object-level Tags section of the base-table Properties modal, alongside the field-specific `AlterTableProperty` writes. |
...
| `sequence.go` | `AlterSequence` (free-form `ALTER SEQUENCE … <clause>` for SET INCREMENT, SET/UNSET COMMENT, RENAME TO — note `START WITH` is not alterable) and `ListAccountSequences` (`SHOW SEQUENCES IN ACCOUNT`, backs the sequence-default picker in the Column Properties modal — a `<seq>.NEXTVAL` reference is the only default `ALTER … ALTER COLUMN … SET DEFAULT` accepts on an existing column); CREATE builder in `builders.go` delegates to `internal/sequence`. |
| `schema.go` | `AlterSchema` (free-form `ALTER SCHEMA <db>.<schema> <clause>` for SET/UNSET COMMENT / DATA_RETENTION_TIME_IN_DAYS / MAX_DATA_EXTENSION_TIME_IN_DAYS / DEFAULT_DDL_COLLATION, ENABLE/DISABLE MANAGED ACCESS, RENAME TO — **two-level** qualify, so it does not reuse the three-level `alterObject` helper) and `GetSchemaParameters` (`SHOW PARAMETERS IN SCHEMA`, the fallback source for the properties panel's parameters SHOW SCHEMAS omits). |
| `database.go` | `AlterDatabase` (free-form `ALTER DATABASE "<db>" <clause>` — **one-level**, so simpler than `AlterSchema`; drives every SET/UNSET plus RENAME TO, SWAP WITH, SET/UNSET TAG, and the ENABLE/DISABLE REPLICATION/FAILOVER, REFRESH, PRIMARY verbs from the `DatabasePropertiesModal`), `GetDatabaseParameters` (`SHOW PARAMETERS IN DATABASE`, the fallback source for the parameters SHOW DATABASES omits), and `ListEventTables` (account-wide `SHOW EVENT TABLES`, fully-qualified names — for the modal's `EVENT_TABLE` picker). |
...
| `tag.go` | `AlterTag` (free-form `ALTER TAG … <clause>` for RENAME/SET/UNSET COMMENT/ADD/DROP/UNSET ALLOWED_VALUES/SET/UNSET PROPAGATE/UNSET ON_CONFLICT/SET/UNSET MASKING POLICY) and `GetTagReferences` (lists where the tag is applied via `SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES`). The centralized Tag Management view adds `ListAccountTags` (`SHOW TAGS IN ACCOUNT`), `GetAllTagReferences` (account-wide `TAG_REFERENCES`, `OBJECT_DELETED IS NULL`, scanned with `LIMIT maxTagReferenceRows+1` and trimmed to 10 000 with `QueryResult.Truncated` set when the extra row returns), and `SetObjectTag` / `UnsetObjectTag` (apply / retag / remove a tag on another object via `tag.BuildAlterObjectTagSql`, given a `tag.ObjectTagRef`). The per-object **Tag References** browser and the Tags section of every object Properties modal share `GetObjectTagReferences` (the object's own tags via the no-latency `INFORMATION_SCHEMA.TAG_REFERENCES('<fqn>','<domain>')` table function, incl. the inheritance `LEVEL`; takes an `args` parameter-type list so procedures / functions carry their overload signature). Account-level domains (`WAREHOUSE`, `ROLE`, `USER`, `INTEGRATION`, `ACCOUNT`, `COMPUTE POOL`, `NETWORK POLICY` — see `accountLevelTagDomain`) carry a bare object name and run inside a resolved host database via the shared `accountLevelFuncDatabase` helper, since they don't live in any one database's `INFORMATION_SCHEMA`. It is joined by `GetColumnTagReferences` (per-column tags via `INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('<fqn>','<domain>')`; the frontend filters this to a single column for a column-node context menu). Both fold the browser kind onto the narrow domain set the table functions accept via `tagReferenceDomain` (specialized tables → `TABLE`, materialized view → `VIEW`, function variants → `FUNCTION`); `tagReferenceObjectName` builds the object-name literal per domain so container nodes work too — `DATABASE` is a bare `"db"`, `SCHEMA` is `"db"."schema"`, callables append their signature. `SetObjectTag` / `UnsetObjectTag` resolve a column's parent kind (table vs view) via `Client.GetObjectTableType` when it isn't supplied, so a view column emits `ALTER VIEW … ALTER COLUMN`. CREATE builder in `builders.go` delegates to `internal/tag`. |
...
| `joinpolicy.go` | `AlterJoinPolicy` (free-form `ALTER JOIN POLICY … <clause>` for RENAME/SET BODY/SET/UNSET COMMENT/SET/UNSET TAG) and `GetJoinPolicyReferences` (lists the tables/views the policy is applied to via `SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES`, `POLICY_KIND = 'JOIN_POLICY'`); the properties Tags editor reads via the shared `GetObjectTagReferences` (`tag.go`). CREATE builder in `builders.go` delegates to `internal/joinpolicy`. |
| `privacypolicy.go` | `AlterPrivacyPolicy` (free-form `ALTER PRIVACY POLICY … <clause>` for RENAME/SET BODY/SET/UNSET COMMENT/SET/UNSET TAG) and `GetPrivacyPolicyReferences` (lists the tables/views the policy is applied to via `SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES`, `POLICY_KIND = 'PRIVACY_POLICY'`); the properties Tags editor reads via the shared `GetObjectTagReferences` (`tag.go`). CREATE builder in `builders.go` delegates to `internal/privacypolicy`. |
| `storagelifecyclepolicy.go` | `AlterStorageLifecyclePolicy` (free-form `ALTER STORAGE LIFECYCLE POLICY … <clause>` for RENAME/SET BODY/SET ARCHIVE_TIER/SET/UNSET ARCHIVE_FOR_DAYS/SET/UNSET COMMENT/SET/UNSET TAG) and `GetStorageLifecyclePolicyReferences` (lists the tables the policy is applied to via `SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES`, `POLICY_KIND = 'STORAGE_LIFECYCLE_POLICY'`); the properties Tags editor reads via the shared `GetObjectTagReferences` (`tag.go`). CREATE builder in `builders.go` delegates to `internal/storagelifecyclepolicy`. |
| `passwordpolicy.go` | `AlterPasswordPolicy` (free-form `ALTER PASSWORD POLICY … <clause>` for RENAME/SET/UNSET each `PASSWORD_*` parameter/SET/UNSET COMMENT/SET/UNSET TAG), `DescribePasswordPolicy` (`DESCRIBE PASSWORD POLICY` — one `property/value/default` row per parameter, which `SHOW PASSWORD POLICIES` omits) and `GetPasswordPolicyReferences` (lists the users/account the policy is attached to via `SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES`, `POLICY_KIND = 'PASSWORD_POLICY'`) and `ListAccountPasswordPolicies` (`SHOW PASSWORD POLICIES IN ACCOUNT`, backs the user-modal password-policy picker); CREATE builder in `builders.go` delegates to `internal/passwordpolicy`. |
| `sessionpolicy.go` | `AlterSessionPolicy` (free-form `ALTER SESSION POLICY … <clause>` for RENAME/SET/UNSET each `SESSION_*` timeout/`ALLOWED_SECONDARY_ROLES`/`BLOCKED_SECONDARY_ROLES`/SET/UNSET COMMENT/SET/UNSET TAG), `DescribeSessionPolicy` (`DESCRIBE SESSION POLICY` — a single row whose columns carry the timeout values and `allowed_secondary_roles`, which `SHOW SESSION POLICIES` omits), `ParseSecondaryRoles` / `FormatSecondaryRoles` / `ReconcileSecondaryRoles` (pure helpers delegating to the matching `snowflake.*` functions, so the create / properties modals parse a DESCRIBE secondary-role cell, serialize a list into the `( 'ALL' | <role>, … )` SQL grammar, and enforce the `ALL`-vs-role-list exclusivity via one shared Go implementation rather than re-deriving any of it in TypeScript) and `GetSessionPolicyReferences` (lists the users/account the policy is attached to via `SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES`, `POLICY_KIND = 'SESSION_POLICY'`) and `ListAccountSessionPolicies` (`SHOW SESSION POLICIES IN ACCOUNT`, backs the user-modal session-policy picker); CREATE builder in `builders.go` delegates to `internal/sessionpolicy`. |
...
| `model.go` | `AlterModel` (free-form `ALTER MODEL … <clause>` covering the full grammar: SET COMMENT/DEFAULT_VERSION, UNSET COMMENT, SET/UNSET TAG, RENAME TO, per-version `VERSION … SET/UNSET ALIAS`, `MODIFY VERSION … SET COMMENT/METADATA`, `ADD VERSION … FROM MODEL/stage`, `DROP VERSION`), `ListModelVersions` (`SHOW VERSIONS IN MODEL`), and `ListModels` (`SHOW MODELS IN ACCOUNT` → quoted FQNs, backs the source-model picker); the properties Tags editor reads via the shared `GetObjectTagReferences` (`tag.go`). CREATE builder in `builders.go` delegates to `internal/model`. GET_DDL is not supported for models. |
...
| `dataset.go` | `AlterDataset` (free-form `ALTER DATASET … <clause>` covering the entire mutable surface — version management only: `ADD VERSION '<name>' FROM ( <query> ) [PARTITION BY …] [COMMENT …] [METADATA …]` and `DROP VERSION '<name>'`; no RENAME / COMMENT / TAG) and `ListDatasetVersions` (`SHOW VERSIONS IN DATASET`); CREATE builder in `builders.go` delegates to `internal/dataset`. GET_DDL is not supported for datasets. |
| `gateway.go` | `AlterGateway` (`ALTER GATEWAY … FROM SPECIFICATION` — updating the traffic-split spec is the **entire** mutable surface; no RENAME / COMMENT / TAG) and `DescribeGateway` (`DESCRIBE GATEWAY` → the ingress URL + `spec` that `SHOW GATEWAYS` omits); CREATE builder in `builders.go` delegates to `internal/gateway`. GET_DDL is not supported for gateways. |
| `contact.go` | `AlterContact` (free-form `ALTER CONTACT … <clause>` for RENAME / SET method / SET COMMENT — there is no UNSET); CREATE builder + `FormatContactUsers` (the `('u1','u2')` USERS value) in `builders.go` delegate to `internal/contact`. A contact names a notification target (mutually-exclusive USERS / EMAIL_DISTRIBUTION_LIST / URL). GET_DDL **is** supported (object_type `'CONTACT'`). |
...
| `semanticview.go` | `AlterSemanticView` (free-form `ALTER SEMANTIC VIEW` — rename / set-unset comment / set-unset tag; the definition body is changed via CREATE OR REPLACE, not ALTER), `DescribeSemanticView` (`DESCRIBE SEMANTIC VIEW` → one row per logical table / relationship / dimension / fact / metric property), `ListSemanticDimensions` / `ListSemanticFacts` / `ListSemanticMetrics` (the `SHOW SEMANTIC …` commands), and `ListSemanticDimensionsForMetric` (`SHOW SEMANTIC DIMENSIONS … FOR METRIC`); the properties Tags editor reads via the shared `GetObjectTagReferences` (`tag.go`). CREATE builder in `builders.go` delegates to `internal/semanticview`. GET_DDL **is** supported (object_type `'SEMANTIC VIEW'`). |
| `git.go` | Git repository browsing, filtering, config persistence, and **index staging** (`GitStageFile`/`GitUnstageFile`/`GitStageAll`/`GitUnstageAll`/`GitDiscardFile`); delegates to `internal/gitrepo`. **Override windows** (`workdirOverridden`): `GetGitConfig` blanks the per-repo `RemoteURL`/`Branch` (so git ops fall back to the actual folder's live status instead of repointing the main window's origin) and returns `cachedExportDir` as `ExportDir`; `SaveGitConfig` persists **only** the per-repo/instance fields (`ExportDir`/`RemoteURL`/`Branch`) — nothing in an override window. The shared fields each have a dedicated atomic, field-scoped writer so a whole-struct save of a stale snapshot (esp. from a second process) can't revert another window's edit: `SaveGitAuthor`, `SaveGitExportPathTemplate`, `SaveGitExportOverloadNaming`, and `AddRecentDir`/`ClearRecentDirs` (`unionRecentDirs`, newest-first, capped 8). All route through `config.Update` (process-scoped locked read-modify-write; writes are atomic temp+rename — see `internal/config/README.md`). `GitLoginWithOAuth` emits the authorization URL via the `git:oauth-url` Wails event (the frontend opens/copies it) rather than forcing a browser; it runs under a per-flow cancelable context so `GitCancelOAuth` (called when the auth dialog is dismissed) frees the loopback server + port 3456 instead of leaking it until quit. |
...
| `ddlexport.go` | `ExportDatabaseDDL`, `ExportAllDatabasesDDL(outputDir, databases, DDLExportOptions)`, `ExportAccountObjectsDDL`, `GetERDiagramData`. `DDLExportOptions` carries the pre-export dialog choices (object-type/schema filters, skip-existing, per-export path template, per-export overload naming — a `ddl.OverloadNaming` string, per-export `dollarQuoteBodies` toggle, warehouse — switched via `client.UseWarehouse` and restored after); zero value = historical behavior. Both naming fields fall back to their persisted `config.Git` preference when the dialog sends an empty string. Contain goroutine orchestration and `ddl:progress` event emission — not thin delegators. |
...
| `users.go` | User/role management IPC: `ListUsers`, `GetUserDDL`, `AlterUserProperty`, plus the non-property `ALTER USER` action delegators `ResetUserPassword` (returns Snowflake's status row — the single-use reset URL — so the UI can show it copyably), `RenameUser`, `AbortAllUserQueries`, `ListUserMfaMethods` (`SHOW MFA METHODS FOR USER` — the enrolled-factor list backing MFA removal; the `name` column is the removal identifier, not the factor `type`; the FOR USER clause needs ACCOUNTADMIN) / `RemoveUserMfaMethod`, `Set/UnsetUserPolicy`, `Set/UnsetUserTags`, `Add/RemoveUserDelegatedAuth` (all delegate to `internal/users`), `GetUserTagReferences` (`INFORMATION_SCHEMA.TAG_REFERENCES` object domain USER) and `GetUserPolicyReferences` (`INFORMATION_SCHEMA.POLICY_REFERENCES`, `REF_ENTITY_DOMAIN => 'USER'` — the immediate-consistency source for a user's attached authentication/password/session policies, since DESCRIBE USER omits them; both back removable chips in the modal). USER is account-level, so both run inside a resolved database via the shared `accountLevelFuncDatabase` helper (session's current database, else any accessible one — common for account admins), plus key generation `CheckAvailableKeyTools` / `GenerateKeyPair` (delegates to `internal/keypair`). RSA public keys are registered through `AlterUserProperty(name, "rsaPublicKey"\|"rsaPublicKey2", value)` — there is no separate set-public-key IPC. |

...
| `users.go` | User/role management IPC: `ListUsers`, `GetUserDDL`, `AlterUserProperty`, plus the non-property `ALTER USER` action delegators `ResetUserPassword` (returns Snowflake's status row — the single-use reset URL — so the UI can show it copyably), `RenameUser`, `AbortAllUserQueries`, `ListUserMfaMethods` (`SHOW MFA METHODS FOR USER` — the enrolled-factor list backing MFA removal; the `name` column is the removal identifier, not the factor `type`; the FOR USER clause needs ACCOUNTADMIN) / `RemoveUserMfaMethod`, `Set/UnsetUserPolicy`, `Set/UnsetUserTags`, `Add/RemoveUserDelegatedAuth` (all delegate to `internal/users`), and `GetUserTagReferences` (`INFORMATION_SCHEMA.TAG_REFERENCES` object domain USER — immediate-consistency read backing the removable-tag chips; USER is account-level so the result is database-independent, but the table function needs to run inside some database's INFORMATION_SCHEMA: the session's current database is used, falling back to any accessible database when none is selected — common for account admins), plus key generation `CheckAvailableKeyTools` / `GenerateKeyPair` (delegates to `internal/keypair`). RSA public keys are registered through `AlterUserProperty(name, "rsaPublicKey"\|"rsaPublicKey2", value)` — there is no separate set-public-key IPC. |

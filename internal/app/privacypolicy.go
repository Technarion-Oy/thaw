...
// AlterPrivacyPolicy runs an ALTER PRIVACY POLICY statement for the given policy.
...
// policy identifier.
func (a *App) AlterPrivacyPolicy(database, schema, name, clause string) error {
...
// 'PRIVACY_POLICY'. The view requires governance privileges (e.g. the
...
func (a *App) GetPrivacyPolicyReferences(database, schema, name string) (*snowflake.QueryResult, error) {
	if a.client == nil {
		return nil, apperrors.ErrNotConnected
	}
	query := fmt.Sprintf(
		"SELECT REF_DATABASE_NAME, REF_SCHEMA_NAME, REF_ENTITY_NAME, REF_ENTITY_DOMAIN, POLICY_STATUS "+
			...
			"WHERE POLICY_DB = '%s' AND POLICY_SCHEMA = '%s' AND POLICY_NAME = '%s' AND POLICY_KIND = 'PRIVACY_POLICY' "+

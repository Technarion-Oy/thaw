...
| `PushParams` | All fields needed for `CommitAndPush`: dir, remoteURL, branch, authMethod, token, message, author, files, `stagedOnly`. When `stagedOnly` is set, the commit runs over the existing index (nothing is staged first). |
| `StageFile` / `UnstageFile` / `StageAll` / `UnstageAll` | Manipulate the git index (whole-file granularity — go-git has no `git add -p`). `UnstageFile` uses a path-constrained mixed reset so the index stat-cache is refreshed and status re-detects the file. |

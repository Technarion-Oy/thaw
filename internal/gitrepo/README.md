...
| Symbol | Description |
|---|---|
| `RepoStatus` | `{ isRepo, branch, modified, added, deleted, staged[], unstaged[], stagedTotal, unstagedTotal, changedPaths, hasRemote, remoteURL, ahead, totalChanged }`. `staged`/`unstaged` are `FileChange` lists distinguishing the index side (git add'ed) from the worktree side; a partially-staged file appears in both. `changedPaths` is an **uncapped** `map[path]ChangedFile{status, isNew}` (worktree status wins for the letter, else staging) used by the file-explorer color overlay and discard prompts so the whole tree is covered even when the capped arrays can't. `isNew` is authoritative — true when the path has no committed version (added/untracked/rename-or-copy destination), i.e. discard deletes rather than reverts. The flat `modified/added/deleted` arrays are kept as a legacy union. |

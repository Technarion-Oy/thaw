...
func (s *session) updateMode(ctx context.Context, newMode string) error {
	...
	// pipeline tools (preview_stage_file), and the Streamlit deploy tool are
	// mode-specific.
	...
	registerPipelineModeTools(s.server, s.client, newMode)
	registerStreamlitModeTools(s.server, s.client, s.cfg.WorkspaceRoot, newMode)

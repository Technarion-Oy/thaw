...
// local filesystem and git repositories only. Always registered in every
// execution mode (metadata, readonly, explain_only).
func registerWorkspaceTools(srv *mcpsdk.Server) {

	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		Name:        "git_status",
		Description: "Return the git status for a directory: branch, modified/added/deleted files, remote info, ahead count. Non-repo directories return isRepo=false.",
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		...
		Description: "List all local and remote branches in the git repository at the given directory.",
	}, func(_ context.Context, _ *mcpsdk.CallToolRequest, in dirInput) (*mcpsdk.CallToolResult, any, error) {
		if in.Dir == "" {
			return nil, nil, fmt.Errorf("dir is required")
		}
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		...
		Description: "Return the content of a file as it exists in the HEAD commit. Returns empty string for new/untracked files.",
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		Name:        "list_directory",
		Description: "List the direct children (files and directories) of a directory, with name, path, size, and type.",
	}, func(_ context.Context, _ *mcpsdk.CallToolRequest, in dirInput) (*mcpsdk.CallToolResult, any, error) {
		if in.Dir == "" {
			return nil, nil, fmt.Errorf("dir is required")
		}
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		Name:        "read_file",
		Description: "Read the content of a file (up to 50 KB). Returns the text content or an error if the file cannot be read.",
	}, func(_ context.Context, _ *mcpsdk.CallToolRequest, in pathInput) (*mcpsdk.CallToolResult, any, error) {
		if in.Path == "" {
			return nil, nil, fmt.Errorf("path is required")
		}
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		Name: "search_files",
		Description: "Search for a query string (or regex) across all files in a directory, recursively. " +
			"Returns matching lines with file path, line number, and match positions. Hidden directories are skipped.",

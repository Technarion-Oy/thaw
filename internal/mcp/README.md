...
| `metadata_cache.go` | `metadataCache` — short-lived per-session catalog cache (databases / schemas / objects / column types / foreign keys) keyed by qualified name with a `metadataCacheTTL` (5 s) window and `singleflight` in-flight dedup, in front of the `metadataSource` interface (satisfied by `*snowflake.Client`). Built once per session in `registerDiagTools`/`registerTabTools` and shared by `validate_sql` and `suggest_join_conditions`, so an AI client refining SQL in a tight loop does not re-issue identical metadata queries (issue #355). `SessionContext` is intentionally never cached; only successful fetches are stored |
...
**Metadata caching & the phase-2 signal (issue #355)**: The editor avoids per-keystroke refetches via its warm object-store cache; the MCP path gets the equivalent from a short-lived `metadataCache` (`metadata_cache.go`) built once per session and shared by `validate_sql` and `suggest_join_conditions`. It memoizes `ListDatabases`/`ListSchemas`/`ListObjects`/`GetTableColumnsWithTypes`/`GetTableForeignKeys` keyed by qualified name for `metadataCacheTTL` (5 s), with `singleflight` collapsing concurrent identical fetches, so an AI client iterating on one query does not re-issue the full metadata set against the live account on every call. `SessionContext` is never cached (it is cheap and changes on `USE`); errors are never cached (a transient failure never poisons the window). Separately, `validate_sql` now returns a structured `validateSqlResult` — `{markers, schemaAware, schemaAwareSkippedReason}` — instead of a bare marker array: when phase 2 is skipped (no connection, or a `ListDatabases`/metadata error, previously logged only at `Debug` and silently discarding all phase-2 markers), `schemaAware` is `false` and `schemaAwareSkippedReason` carries the cause, so an empty `markers` list only means "checked and clean" when `schemaAware` is `true`.

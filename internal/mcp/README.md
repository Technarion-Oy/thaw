...
| `streamlit_tools.go` | `registerStreamlitTools` — Streamlit builder/template tools (`build_create_streamlit_sql`, `list_streamlit_templates` always registered; `create_streamlit_from_template` workspace-gated); `registerStreamlitModeTools` — `deploy_streamlit`, the only Snowflake-mutating tool, gated on a workspace root **and** `readonly` mode. Delegates to `streamlit` and `streamlittemplate`; `validateDeployStreamlitFields`/`validateDeployStreamlitPaths` hold the required-input and sandbox rules |
| `gate.go` | EXPLAIN precompilation gate: `queryRunner` interface, `CheckGate` (3-layer validation), `checkExplainPlan`, `readOnlyOps` allow-list, `extractOperations`, `isUSEStatement` (tokenizer-based via `sqltok.FirstToken`) |
...
| `streamlit_tools_test.go` | Unit tests for Streamlit tools (gating matrix across mode × workspace, `deploy_streamlit` appearing/disappearing across `updateMode` switches, input validation, template attribution in both result shapes, sandbox tests for `create_streamlit_from_template` including a symlink escape, nil-client handling, and the deploy path rules via `validateDeployStreamlitPaths`) |
| `mcp_test.go` | SSE round-trip test (external client lists tools), port-allocation test, diagnostics tool tests (including the `validate_sql` `schemaAware` result shape), mode-gating tests |
| `metadata_cache_test.go` | Unit tests for `metadataCache` (cache hits within TTL, per-key isolation, TTL expiry via injected clock, errors not cached, `singleflight` dedup, `SessionContext` never cached) using a call-counting `metadataSource` fake |
...
3. **USE statement check** — `isUSEStatement(sql)` rejects USE statements with a descriptive error (use the dedicated context-switching tools instead). Detection is tokenizer-based: `sqltok.FirstToken(sql) == "USE"`, so every comment style Snowflake accepts (`--`, `//`, nested `/* */`) and any separator between `USE` and its operand are handled by the shared tokenizer rather than a keyword prefix match. It remains an early check for traceability; the EXPLAIN gate is the authoritative backstop.
...
6. **readonly mode** — wrap the query with `injectLimit` (`SELECT * FROM (\n<query>\n) AS _mcp_limit LIMIT 100`) to prevent full-table scans, execute, and cap at `maxMCPResultRows` (1000). The inner query sits on its own line: a query ending in a trailing `--` line comment would otherwise swallow the closing paren and the `LIMIT`, turning a valid query into a Snowflake syntax error.
...
Safety rails on `deploy_streamlit` (issue #847): `database`, `schema` and `name` are required and never defaulted from the session context; `orReplace` is an explicit opt-in so an existing app is never silently overwritten; `localDir` is validated against the workspace root and `mainFile` must resolve inside `localDir`; and the executed `CREATE STREAMLIT` statement is returned in the result so the client can show exactly what ran. Those two path checks cover only the two paths the tool names — the sandbox for the rest of the tree is enforced one layer down, in `stage.planDirUploads`, which skips any symlink under `localDir` that doesn't resolve to a regular file inside it. That matters because `PUT` opens each local path *through* a link, so an unchecked one is an arbitrary-file read out of the workspace and into a stage the deployed app can serve back. `create_streamlit_from_template` validates `destDir` with `filesystem.ValidatePathOrAncestorInsideOrEqual` (the destination legitimately may not exist yet — `DownloadTemplate` creates it and refuses a non-empty folder).
...
**`validate_sql` ↔ editor parity**: `validate_sql` and the frontend editor share a single Go orchestrator, `sqleditor.Diagnose` (`internal/sqleditor/orchestrator.go`), which owns both the phase ordering and the table-ref resolution semantics — including the "trust a fully-qualified ref only after catalog verification, else drop it so existence validation flags it" rule the editor uses. The MCP path calls `Diagnose` directly through a `clientSchemaProvider` adapter; the editor drives the same validators through granular IPC methods because it layers an incremental, offline-first catalog warm-up on top (fetch missing schemas/objects, then re-run) that can't run server-side. The markers therefore **approximate** the editor and can still differ for the same SQL — the tool Description says so — but the resolution logic no longer drifts by construction. This closes issue #354 (was: a hand-mirrored copy kept in sync only by `NOTE:` comments).

**Metadata caching & the phase-2 signal (issue #355)**: The editor avoids per-keystroke refetches via its warm object-store cache; the MCP path gets the equivalent from a short-lived `metadataCache` (`metadata_cache.go`) built once per session in `buildServer` and shared by every tool that runs the diagnostics pipeline — `validate_sql`, `suggest_join_conditions`, and `open_sql_tab` (which validates before delivering SQL to a tab), so the common validate-then-deliver handoff reuses the metadata it just fetched. It memoizes `ListDatabases`/`ListSchemas`/`ListObjects`/`GetTableColumnsWithTypes`/`GetTableForeignKeys` keyed by qualified name for `metadataCacheTTL` (5 s), with `singleflight` collapsing concurrent identical fetches, so an AI client iterating on one query does not re-issue the full metadata set against the live account on every call. The shared fetch runs on a context **detached** from any single caller (`context.WithoutCancel` + a `metadataFetchTimeout` bound), so one caller canceling (a client-side timeout/abort) cannot abort the in-flight fetch that other deduped callers are awaiting. `SessionContext` is never cached (it is cheap and changes on `USE`); errors are never cached (a transient failure never poisons the window); expired entries are lazily evicted so the map stays bounded across a long session. Separately, `validate_sql` now returns a structured `validateSqlResult` — `{markers, schemaAware, schemaAwareSkippedReason}` — instead of a bare marker array: when phase 2 is skipped (no connection, or a `ListDatabases`/metadata error, previously logged only at `Debug` and silently discarding all phase-2 markers), `schemaAware` is `false` and `schemaAwareSkippedReason` carries the cause, so an empty `markers` list only means "checked and clean" when `schemaAware` is `true`.

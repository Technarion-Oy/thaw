...
- `ListExtendedObjects(ctx, db, schema)` — runs the per-type SHOW commands not covered by `SHOW OBJECTS` (DYNAMIC TABLE, EXTERNAL TABLE, MATERIALIZED VIEW, PROCEDURE, FUNCTION, TASK, STREAM, STAGE, FILE FORMAT, PIPE, NOTEBOOK, SECRET, GIT REPOSITORY, DBT PROJECT) concurrently; failures per-type are silently skipped

...
- `ListExtendedObjects(ctx, db, schema)` — runs the per-type SHOW commands not covered by `SHOW OBJECTS` (DYNAMIC TABLE, EXTERNAL TABLE, MATERIALIZED VIEW, ALERT, TAG, MASKING POLICY, NETWORK RULE, PROCEDURE, FUNCTION, TASK, STREAM, STAGE, FILE FORMAT, PIPE, NOTEBOOK, SECRET, GIT REPOSITORY, DBT PROJECT) concurrently; failures per-type are silently skipped. Alerts, tags, masking policies, and network rules are not surfaced by `SHOW OBJECTS`, so — unlike dynamic / external tables and materialized views — they need no dedupe pass. Masking policies map to the `GET_DDL` object type `POLICY` (which covers all policy kinds), so `buildGetDDLQuery` normalizes the SHOW kind `"MASKING POLICY"` → `POLICY`; likewise `"NETWORK RULE"` → `NETWORK_RULE`

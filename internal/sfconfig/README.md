...
| Symbol | Description |
|---|---|
| `Connection` | A single named profile: `account`, `user`, `password`, `role`, `warehouse`, `database`, `schema`, `authenticator`, `passcode`, `oktaUrl`, `privateKeyPath`, `privateKeyPassphrase`, plus the token/OAuth2/WIF auth fields (`token`, `tokenFilePath`, `oauthClientId`, `oauthClientSecret`, `oauthTokenRequestUrl`, `oauthAuthorizationUrl`, `oauthRedirectUri`, `oauthScope`, `enableSingleUseRefreshTokens`, `workloadIdentityProvider`, `workloadIdentityEntraResource`, `workloadIdentityImpersonationPath`), plus the forward-proxy fields (`proxyHost`, `proxyPort`, `proxyUser`, `proxyPassword`, `proxyProtocol`, `noProxy`). TOML keys are snake_case (e.g. `token_file_path`, `oauth_client_id`, `workload_identity_provider`, `proxy_host`, `no_proxy`); `enableSingleUseRefreshTokens` is written as an unquoted TOML bool (`enable_single_use_refresh_tokens = true`, emitted only when true), and `proxyPort` as an unquoted TOML int (`proxy_port = 8080`, emitted only when non-zero). |

...
func TestParseAlterSequence(t *testing.T) {
	...
	assertInvalid(t, (*Validator).ParseAlterSequence,
		...
		`DROP SEQUENCE s`,
		`ALTER SEQUENCE`,
		`ALTER SEQUENCE s SET BOGUS = 'x'`,
		`ALTER SEQUENCE s UNSET ORDER`,
	)
}
...
func TestParseAlterService(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterService,
		...
		`ALTER SERVICE svc SET AUTO_RESUME = TRUE EXTERNAL_ACCESS_INTEGRATIONS = (eai1, eai2)`,
		`ALTER SERVICE svc SET TAG t = 'v'`,
		`ALTER SERVICE svc UNSET MIN_INSTANCES, COMMENT`,
	)
	assertInvalid(t, (*Validator).ParseAlterService,
		``,
		`ALTER SERVICE`,
		`CREATE SERVICE svc SUSPEND`,
		`ALTER SERVICE svc SET BOGUS = 'x'`,
		`ALTER SERVICE svc UNSET BOGUS`,
	)
}
...
func TestParseAlterSessionPolicy(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterSessionPolicy,
		...
		`ALTER SESSION POLICY sp SET SESSION_MAX_LIFESPAN_MINS = 60 COMMENT = 'x'`,
		`ALTER SESSION POLICY sp SET ALLOWED_SECONDARY_ROLES = ('ALL')`,
		`ALTER SESSION POLICY sp SET TAG t1 = 'a', t2 = 'b'`,
		`ALTER SESSION POLICY sp UNSET TAG t1`,
		`ALTER SESSION POLICY sp UNSET SESSION_IDLE_TIMEOUT_MINS, COMMENT`,
	)
	assertInvalid(t, (*Validator).ParseAlterSessionPolicy,
		``,
		`ALTER SESSION sp RENAME TO sp2`,
		`ALTER SESSION POLICY sp SET BOGUS = 1`,
		`ALTER SESSION POLICY sp SET SESSION_IDLE_TIMEOUT_MINS = 'x'`,
		`ALTER SESSION POLICY sp UNSET BOGUS`,
	)
}
...
func TestParseAlterShare(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterShare,
		...
		`ALTER SHARE s ADD ACCOUNTS = a1, a2 SHARE_RESTRICTIONS = FALSE`,
		`ALTER SHARE s SET ACCOUNTS = a1, a2 COMMENT = 'x'`,
		`ALTER SHARE s SET TAG t1 = 'a'`,
		`ALTER SHARE s UNSET TAG t1`,
	)
	assertInvalid(t, (*Validator).ParseAlterShare,
		``,
		`ALTER SHARE`,
		`ALTER SHARE s SET BOGUS = 1`,
		`ALTER SHARE s UNSET BOGUS`,
	)
}
...
func TestParseAlterSnapshotPolicy(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterSnapshotPolicy,
		...
		`ALTER SNAPSHOT POLICY p SET SCHEDULE = '5 MINUTE' EXPIRE_AFTER_DAYS = 7`,
		`ALTER SNAPSHOT POLICY p SET TAG t1 = 'a'`,
		`ALTER SNAPSHOT POLICY p UNSET TAG t1, t2`,
	)
	assertInvalid(t, (*Validator).ParseAlterSnapshotPolicy,
		``,
		`ALTER SNAPSHOT p RENAME TO p2`,
		`ALTER SNAPSHOT POLICY p SET BOGUS = 1`,
		`ALTER SNAPSHOT POLICY p UNSET BOGUS`,
	)
}
...
func TestParseAlterStream(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterStream,
		...
		`ALTER STREAM s SET TAG t1 = 'a', t2 = 'b'`,
		`ALTER STREAM s UNSET TAG t1, t2`,
	)
	assertInvalid(t, (*Validator).ParseAlterStream,
		``,
		`ALTER STREAM`,
		`ALTER STREAM s SET BOGUS = 1`,
		`ALTER STREAM s UNSET BOGUS`,
	)
}
...
func TestParseAlterApplicationService(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterApplicationService,
		...
		`ALTER APPLICATION SERVICE svc SET AUTO_SUSPEND_SECS = 60 QUERY_WAREHOUSE = wh`,
		`ALTER APPLICATION SERVICE svc SET EXTERNAL_ACCESS_INTEGRATIONS = (a, b)`,
		`ALTER APPLICATION SERVICE svc UNSET AUTO_RESUME, COMMENT`,
	)
	assertInvalid(t, (*Validator).ParseAlterApplicationService,
		``,
		`ALTER SERVICE svc SUSPEND`,
		`ALTER APPLICATION SERVICE svc SET BOGUS = 1`,
		`ALTER APPLICATION SERVICE svc UNSET BOGUS`,
	)
}
...
func TestParseAlterArtifactRepository(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterArtifactRepository,
		...
		`ALTER ARTIFACT REPOSITORY ar SET TAG t1 = 'a', t2 = 'b'`,
	)
	assertInvalid(t, (*Validator).ParseAlterArtifactRepository,
		``,
		`ALTER REPOSITORY ar SET COMMENT = 'x'`,
		`ALTER ARTIFACT REPOSITORY ar SET BOGUS = 1`,
		`ALTER ARTIFACT REPOSITORY ar UNSET BOGUS`,
	)
}

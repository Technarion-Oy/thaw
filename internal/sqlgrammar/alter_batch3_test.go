...
func TestParseAlterSemanticView(t *testing.T) {
	assertValid(t, (*Validator).ParseAlterSemanticView,
		...
		`ALTER SEMANTIC VIEW "DB"."SC"."v1" SET MAX_STALENESS = 300`,
		`ALTER SEMANTIC VIEW sv UNSET MAX_STALENESS`,
		`ALTER SEMANTIC VIEW sv ADD MATERIALIZATION mv1 WAREHOUSE = wh AS DIMENSIONS customers.region METRICS orders.revenue`,
		`ALTER SEMANTIC VIEW sv ADD MATERIALIZATION mv1 WAREHOUSE = wh REFRESH_MODE = FULL IMMUTABLE WHERE (region = 'US') AS DIMENSIONS customers.region, orders.segment METRICS orders.revenue WHERE (revenue > 0)`,
		`ALTER SEMANTIC VIEW sv DROP MATERIALIZATION mv1`,
		`ALTER SEMANTIC VIEW sv SUSPEND MATERIALIZATION "mv1"`,
		`ALTER SEMANTIC VIEW sv RESUME MATERIALIZATION mv1`,
		`ALTER SEMANTIC VIEW sv REFRESH MATERIALIZATION mv1`,
	)
	assertInvalid(t, (*Validator).ParseAlterSemanticView,
		...
		`ALTER SEMANTIC VIEW sv ADD MATERIALIZATION mv1 AS DIMENSIONS customers.region METRICS orders.revenue`,
		`ALTER SEMANTIC VIEW sv ADD MATERIALIZATION mv1 WAREHOUSE = wh REFRESH_MODE = BOGUS AS DIMENSIONS customers.region METRICS orders.revenue`,
		`ALTER SEMANTIC VIEW sv DROP MATERIALIZATION`,
	)
}

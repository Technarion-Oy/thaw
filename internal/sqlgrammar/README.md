...
| File | Purpose |
|------|---------|
| `engine.go` | The `Validator` type, terminals (`Match`/`MatchKeyword`/`MatchWord`/`MatchOp`), combinators (`Sequence`/`Choice`/`Optional`/`ZeroOrMore`/`unorderedOnce` — the last for order-independent parameter lists where each option may appear at most once, so duplicates are rejected and `ExpectedAt` stops offering a parameter already set), `furthest`/`expected` tracking + `Failure`, and shared helpers (`parseIdentPath` and its part-bounded variant `parseIdentPathN(maxParts)` — used e.g. by `USE DATABASE`/`ROLE`/`WAREHOUSE` (single part) and `USE SCHEMA` (≤2 parts) so a dot-qualified name like `USE DATABASE db.schema` is rejected, issue #765; `parseUseTargetName` further rejects a bare object-type keyword as the name so a dangling `USE DATABASE`/`USE ROLE` is flagged instead of swallowing the keyword, issue #793 B17), `parseScalar` — which accepts a Scripting bind reference `:<name>` and the `IDENTIFIER( … )` wrapper as atoms (`parseBindVar`/`parseIdentifierFn`), `option`, `wordsValue`, `phrase`, `tagClause`, `matchIdentLike` (identifier-or-keyword atom), `parseStageRef` (`@`-prefixed stage path / `IDENTIFIER( … )`), `clusterByClause` (`CLUSTER BY [ LINEAR ] <parens>`), `consumeBalancedParens`, `consumeRest`, `showTrailers`, …) |
| `dispatch.go` | `Recognized()` + `ParseTopLevel()`: a leading-keyword → candidate-rules registry (bulk families by `Parse*` prefix via reflection, DML/misc leaders enumerated explicitly — including the documented `LS`/`RM` abbreviations of `LIST`/`REMOVE` so they are grammar-checked rather than passing silently, issue #793 C); `ExpectedAt(cursorOffset)` — the autocomplete "valid next" accessor; plus `IdentifyStatement()` — the effective-verb classifier that looks past a leading `WITH`/CTE prefix |
...
| `scripting.go` | Snowflake Scripting procedural constructs — a separate grammar layer. `ParseScriptingBlock` is the structural core: `[ DECLARE … ] BEGIN <stmt>; … [ EXCEPTION … ] END`, whose optional trailing handler is `ParseException` (`EXCEPTION WHEN <exc> [ OR <exc> … ] [ {EXIT|CONTINUE} ] THEN <stmt>; … [ WHEN … ]` — `WHEN OTHER` is just the reserved name; invoked via `Optional` from the block, not the statement `Choice` or dispatch). It is dispatched under `BEGIN` (alongside the transaction `ParseBegin`) and `DECLARE`, and recursing as a nested block. `parseScriptingStatement` is the **shared block-body `Choice`** the loop/branch/cursor issues extend by inserting their construct ahead of the permissive statement-span catch-all. `ParseDeclare` models the `DECLARE` section (and standalone rule): one-or-more `;`-terminated declarations, each dispatched by the token after `<name>` — cursor (`CURSOR FOR <query>`), RESULTSET (`RESULTSET [ {DEFAULT|:=} [ASYNC] (<query>) ]`), nested procedure (`PROCEDURE ( … ) RETURNS … AS <block>`), exception (`EXCEPTION [ (<num>, '<msg>') ]`), or the catch-all variable (`<name> [<type>] [ {DEFAULT|:=} <expr> ]`); types/queries/exprs are permissive spans (`:=` is a Colon+`=` token pair). Other constructs (`AWAIT`, `BREAK`/`EXIT`, `CANCEL`, `CASE`, `CLOSE`, `CONTINUE`/`ITERATE`, `FETCH` (`FETCH <cursor> INTO <var> [ , <var> … ]`), `FOR`, `IF`, `LET`, `LOOP`, `NULL`, `OPEN`, `RAISE`, `REPEAT`, `RETURN`, `WHILE` — the latter sixteen wired into the block-body `Choice`; `CASE` covers both the simple `CASE (<operand>) WHEN … THEN …` and searched `CASE WHEN <bool> THEN …` forms with `END [ CASE ]`, its operand/conditions consumed via `consumeExprSpan` and `ELSE` added as a branch-body boundary; `FOR` covers both the cursor-based `FOR <var> IN <cursor> DO <stmt>; … END FOR [<label>]` and counter-based `FOR <var> IN [REVERSE] <start> TO <end> {DO|LOOP} <stmt>; … END {FOR|LOOP} [<label>]` forms, the cursor form tried first so a numeric range falls through, `<start>`/`<end>` consumed via `consumeExprSpan`; `IF` covers `IF <condition> THEN <stmt>; … [ ELSEIF <condition> THEN … ] [ ELSE … ] END IF`, conditions consumed via `consumeExprSpan` up to `THEN` and `ELSEIF`/`ELSE` added as branch-body boundaries; `LET` covers the three assignment forms `LET <name> [<type>] {DEFAULT|:=} <expr>`, `LET <name> CURSOR FOR <query>`, and `LET <name> RESULTSET {DEFAULT|:=} [ASYNC] (<query>)` — like `DECLARE` but the assignment is **required**, the variable branch tried last so `<type>` cannot shadow the `CURSOR`/`RESULTSET` forms; `LOOP` covers the infinite loop `LOOP <stmt>; … END LOOP [<label>]`, exited via `BREAK`/`RETURN`; `NULL` is the no-op statement `NULL`; `OPEN` covers `OPEN <cursor> [ USING (<bind> [, <bind> …]) ]`, opening a cursor by running its query; `RAISE` covers `RAISE [ <exception_name> ]`, raising a named exception (or, with the name omitted, re-raising the current one in a handler) to halt block execution; `REPEAT` covers the post-test loop `REPEAT <stmt>; … UNTIL (<condition>) END REPEAT [<label>]` — body runs at least once, the required parenthesized condition consumed as a balanced-paren span; `RETURN` covers `RETURN <expression>`, exiting the block and returning the expression's value to the caller, the expression consumed as a permissive span up to `;`; `WHILE` covers the pre-test loop `WHILE (<condition>) {DO|LOOP} <stmt>; … END {WHILE|LOOP} [<label>]`, the required parenthesized condition consumed as a balanced-paren span like `REPEAT`; plus the standalone `ParseAssignment` — the variable update `<name> := <expr>` (`:=` only — `walrus`, never `DEFAULT`), tried before the catch-all so it gets a precise rule, …) live here too. **Variable references** (issue #648): a Scripting bind `:<name>` and `IDENTIFIER(:<name>)` are accepted as atoms in `parseScalar`; inside SQL statement bodies (VALUES, WHERE, function args, array-spread bind lists) they are already tolerated by the permissive statement/expression spans, so no per-statement change is needed |
| `ctenames.go` | `CollectCTENames(src, sig)` — the single structural CTE-alias-name scanner (handles `WITH RECURSIVE`, CTE column lists, nested WITH, unterminated bodies mid-typing). Shared by `internal/snowflake` lineage extraction and `internal/sqleditor` table-existence diagnostics (issue #559); callers apply their own normalisation. `CollectCTEDefs(src, sig)` — span-returning sibling walking the same positions; returns each member's `CTEDef` (name plus byte-offset column-list and body spans) so `internal/sqleditor` CTE-projection extraction can slice out the body without its own regex (issue #673) |

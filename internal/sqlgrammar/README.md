...
| `scripting.go` | Snowflake Scripting procedural constructs (`AWAIT`, …) that live inside a `BEGIN … END` block-body statement `Choice` rather than top-level dispatch — a separate grammar layer |

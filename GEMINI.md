...
- **Filesystem Operations**: `internal/filesystem/` — file read/write/delete/rename, directory creation, reveal in platform file manager, and recursive file search. All mutating operations (`DeleteFile`, `DeleteDirectory`, `RenameFile`, `MkDir`, `WriteFileInRoot`) validate that paths are strictly inside an allowed root directory, resolving symlinks to prevent escape. The File Browser sidebar (`frontend/src/components/files/FileBrowser.tsx`) exposes these via a right-click context menu.
- **Cross-Tab Search**: `frontend/src/components/editor/CrossTabSearch.tsx` — search/replace panel that searches across all open tabs (SQL, YAML, Python, notebook cells); opened via `⌘⇧H`; gated behind the `crossTabSearch` feature flag.

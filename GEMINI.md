...
- **Object-Kind Registry**: `internal/objectkind/` — the single canonical list of Snowflake object kinds (canonical KIND, SHOW plural, display label, tree order, `Basic` = returned by `SHOW OBJECTS`, `GET_DDL` object type). Every consumer derives from it: `ListExtendedObjects` and the account-wide search (`internal/snowflake`), `BuildObjectPropertiesQuery` (`internal/objects`), `buildGetDDLQuery` + `DDLUnsupportedKinds`, and — via `go generate ./internal/objectkind/` → `frontend/src/generated/objectKinds.ts` — the sidebar's kind labels, ordering, type-filter options and DDL guard. It is a leaf package (no Thaw imports) so `internal/snowflake` can depend on it. **Adding an object kind = one registry entry + `go generate` + an icon/colour in `components/sidebar/objectIcons.tsx`**; coverage tests (`TestEveryKind*`, `TestGeneratedObjectKindsInSync`, `objectIcons.test.ts`) fail if a consumer is left unwired. Do not hand-edit the generated TS or reintroduce a per-kind `switch`.
...
Thaw uses a centralized feature flag system to gate optional or experimental capabilities. User-visible flags are managed via **View → Enabled Features…**. A flag earns a user-facing toggle only when it hides an admin surface, declutters a whole optional workflow, lets credit-conscious users stop automatic background Snowflake/AI queries, or is experimental/opt-in — basic functionality ships always-on (issue #567 removed a batch of basic-functionality toggles).
...
5. **UI**: Add a corresponding `<FlagRow />` to `frontend/src/components/settings/FeatureFlagsModal.tsx` in the appropriate category, and list the field in `frontend/src/store/featureFlagsStore.ts`.

### Removing a Flag
- **Basic, not admin-lockable** → delete the field from `FeatureFlags`/`DefaultFeatureFlags()`/`MigrateFlags()`, bump `flagsVersion`, remove the `<FlagRow />` and store entries, un-gate every consumer, and run `wails generate module`.
- **Basic, but keep an admin lock** → leave `config.go`/`adminconfig.go`, delete only the `<FlagRow />`, and add the field to `ForceAlwaysOnFlags()` so it's forced on for users while IT policy can still disable it (`loadUserFeatureFlags` runs `ForceAlwaysOnFlags` after `MigrateFlags`, before admin overrides).
...
- **Shift+click range**: `onSelect` also handles `nativeEvent.shiftKey`, selecting every node between `selAnchorKey` (the last Cmd/Ctrl-clicked pivot) and the click. Visible order comes from `flattenVisibleNodes(displayData, expandedSet, …)`, which walks the tree against the controlled `expandedKeys`. The tree wrapper sets `userSelect: none` and `preventDefault`s shift-mousedown so the range click doesn't paint a browser text selection.
- **Selectable kinds**: object nodes (`obj:`) and database nodes (`db:`) only — see `MULTI_SELECT_PREFIXES` / `multiSelectPrefix()`. A selection is always homogeneous: ranges filter to the clicked node's prefix and a Cmd/Ctrl toggle drops keys of the other kind. `selectionKind` (`"obj" | "db" | null`) gates the bulk context-menu entries (`deleteSelectedObjects`, `addSelectedAsInsertSources` vs `exportSelectedDatabases`, `dropSelectedDatabases`).
- **No expand on modified click**: `expandAction="click"` makes rc-tree expand on any click, so `onExpand` returns early when `nativeEvent` carries Cmd/Ctrl/Shift on a multi-selectable node (`expandedKeys` is controlled, so dropping the event suppresses the expand).
...
- Always available for users; admin policy can still force it off via the `insertRow` flag (no user toggle since issue #567).

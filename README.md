...
- **Snowflake Scripting support** — advanced support for Snowflake Scripting (Stored Procedures/UDFs) with distinct syntax coloring for blocks (`DECLARE`/`BEGIN`), control flow, and async management; **right-click → Code Snippets →** opens a cascading hover submenu with snippet templates grouped into seven categories (Block Structure, DECLARE Variables, LET Variables, Conditionals, Loops, Cursors & Resultsets, Async Jobs) — hover over the menu item to expand, click any snippet to insert it at the cursor; snippets automatically match **Editor Preferences** (keyword casing and indentation style); **transparent dollar quoting** treats code inside `$$` as normal SQL for full highlighting and diagnostics
...
- **Live SQL diagnostics** — real-time squiggly-line markers appear 400 ms after each edit; no false positives on well-formed Snowflake SQL:
  ...
  - **Grammar warnings** (yellow) — a Snowflake-dialect PEG parser (`node-sql-parser`) validates the full grammatical structure of SELECT, INSERT, UPDATE, CREATE, DROP, ALTER, and related statements; Snowflake-specific constructs not yet covered by the parser (e.g. `TABLESAMPLE`, `AT (TIMESTAMP =>…)`, `WITHIN GROUP`) are silently skipped to avoid false positives; additionally, 50+ statement types are validated via Go-side regex patterns in the backend (`internal/sqleditor`) to catch structural errors without false positives — including CREATE/ALTER/DROP for STREAM, PIPE, ALERT, PROCEDURE, FUNCTION, DYNAMIC TABLE, HYBRID TABLE, ICEBERG TABLE, EVENT TABLE, NETWORK POLICY, ROW ACCESS POLICY, SESSION POLICY, PASSWORD POLICY, AGGREGATION POLICY, PROJECTION POLICY, PACKAGES POLICY, FILE FORMAT, SHARE, DATASHARE, EXTERNAL VOLUME, TAG, TASK, NOTEBOOK, REPLICATION GROUP, FAILOVER GROUP, COMPUTE POOL, SERVICE, IMAGE REPOSITORY, SECRET, GIT REPOSITORY, APPLICATION PACKAGE, APPLICATION, GRANT/REVOKE with privilege-to-object-type compatibility checks, and SNOWFLAKE.CORTEX.* AI function call validation (unknown Cortex function names are flagged), PIVOT/UNPIVOT clause structural validation (aggregate function, FOR … IN syntax, non-empty value list), MATCH_RECOGNIZE clause structural validation (mandatory PATTERN and DEFINE clauses, ONE ROW/ALL ROWS mutual exclusion, AFTER MATCH SKIP target validation), ASOF JOIN clause structural validation (mandatory MATCH_CONDITION or USING FUNCTION, valid comparison operators, ON/USING clause rejection), multi-table INSERT validation (INSERT ALL/INSERT FIRST conditional and unconditional forms with WHEN/THEN INTO/ELSE, trailing SELECT requirement, INSERT OVERWRITE INTO structural checks), ALTER TABLE ADD/DROP SEARCH OPTIMIZATION validation (ON clause expression types: EQUALITY, SUBSTRING, GEO, FULL_TEXT — unknown types are flagged), ALTER TABLE SWAP WITH validation (missing target, same-table no-op, trailing clauses), and ALTER DYNAMIC TABLE lifecycle command validation (REFRESH, SUSPEND, RESUME, SET, UNSET, SWAP WITH, RENAME TO — including TARGET_LAG value validation and missing-argument checks)
...
- **Result history** — the last 10 successful result sets are kept in memory; a dropdown in the results status bar lets you switch between them (analogous to `LAST_QUERY_ID(-n)`); after a query failure the dropdown becomes a standalone **Previous results** picker — the grid is hidden until a result is explicitly selected, keeping the error visible and unambiguous; click the **pin** icon next to any entry to keep it indefinitely (pinned entries are exempt from the 10-result cap and float to the top of the dropdown); **right-click** any entry and choose **View side by side** to split the results area horizontally — each grid scrolls independently so you can compare different regions of two result sets; the compare panel's query ID and row count appear on a second line of the status bar (right-aligned); close the split with the × button
...
- **Task tree** — tasks inside a schema are displayed as a hierarchy in the sidebar: child tasks appear nested under their predecessor root task; finalizer tasks are shown as the last child of their root task with a purple **Finalizer** badge so the graph structure is visible without opening the graph modal
...
- Right-click an **object** to:
  ...
  - **Insert Mapping** — select a table as an **Insert Target** and one or more tables or views as **Insert Sources** to open a column mapping dialog; sources are displayed side-by-side in a single view; supports both **UNION ALL** and **UNION** combination modes; generates a formatted multi-line `INSERT INTO … SELECT` statement with each column on its own indented line; a **Quote identifiers** toggle (on by default) controls quoting — when off, identifiers that are structurally safe, not Snowflake reserved keywords, and all-uppercase (i.e. were not created with quoted mixed/lower-case names) are emitted without double quotes; reserved keywords (`START`, `END`, `FAIL`, `MAX`, `MIN`, …) and case-sensitive names are always quoted regardless of the toggle
  ...
  - **View Task Graph…** (tasks) — opens an interactive DAG visualisation of the full task graph rooted at the selected task; left-to-right layout is computed automatically via Dagre; each node shows the task name, schedule state (STARTED / SUSPENDED), last-run state badge (Running, Succeeded, Failed, Skipped, Scheduled, Cancelled, Waiting…), and — for completed or failed runs — a completion timestamp (HH:MM:SS for runs today, "Jan 15 HH:MM" for earlier dates); the graph polls Snowflake every 3 seconds and updates node states in place without re-running the layout; a pulsing **Live** indicator and timestamp are shown in the canvas; tasks skipped because a predecessor failed are inferred and shown as Skipped with transitive propagation — correctly overrides a stale Succeeded row from a previous run when the predecessor's failure is more recent; timestamps are suppressed for Skipped nodes since no TASK_HISTORY row exists for the current run; two action buttons are embedded in the top-right of the canvas:
    - **Run Graph** — calls `EXECUTE TASK <root>` immediately; all child nodes switch to "Waiting…" optimistically until the first poll returns with real states
    - **Retry Failed** — calls `EXECUTE TASK <root> RETRY LAST`; enabled only when the last graph run failed or was cancelled and the first attempt was within 14 days; disabled with a descriptive tooltip when conditions are not met
    ...
    - **Export DDL** button — copies the entire graph's DDL as an ordered SQL script (topological order: root first, children in BFS order, finalizer last) to the clipboard; hover the button to toggle "Include SUSPEND/RESUME statements" which wraps the script with `ALTER TASK … SUSPEND` / `ALTER TASK … RESUME` in the correct dependency order
    - **Finalizer task display** — a task created with `FINALIZE = <root>` is shown with a dashed purple border, a purple "Finalizer" badge alongside the STARTED/SUSPENDED schedule state badge, and a dashed purple **finalizes** edge from the root node; the finalizer node is placed at the far right of the layout after all leaf tasks; detected via `GET_DDL` as a reliable fallback when the `task_relations` SHOW TASKS column is unavailable
    - **Right-click any node** to access a context menu:
      ...
      - **Create Finalizer Task…** — opens the Create Task dialog pre-configured for finalizer mode (schedule hidden, FINALIZE pre-filled with the root task name); enabled only on the root node and only when no finalizer task already exists; shown as "(already has one)" when the root already has a finalizer
      - **Delete Task…** — drops the right-clicked task individually; suspends it first if currently STARTED; a confirmation dialog shows the task name before running `DROP TASK IF EXISTS`; disabled for tasks that still have child tasks (label: "remove child tasks first"); if the deleted task was the only task in the graph the modal closes automatically
      - **Export DDL** — copies the single task's DDL to the clipboard
  - **View Run History…** (tasks) — opens a modal showing the chronological execution history for the selected task, queried from `INFORMATION_SCHEMA.TASK_HISTORY()`; a **Segmented** control toggles between "Last 24 Hours" and "Last 7 Days"; an optional auto-refresh polls every 10 seconds; summary chips show counts of succeeded, failed, executing, and skipped runs; for **root tasks** rows are grouped by `SCHEDULED_TIME` into collapsible DAG Runs — expanding a run reveals the individual child task executions with status, start time, duration, and error message; for **child tasks** a flat execution list is shown; also accessible from the **History** button in the Task Properties status bar
...
- **Properties** — right-click any database, schema, or object → **Properties** opens a key/value panel populated by the corresponding `SHOW` command; a search bar filters properties by name in real time; a **Copy** button copies all rows as `property: value` lines; for **tables** the panel includes two additional inline-editable sections:
...
- **Task Properties** (tasks) — right-clicking a task and selecting **Properties** opens a dedicated editable modal covering the full `ALTER TASK` syntax, organised into sections:
  ...
  - **Status** — RESUME / SUSPEND toggle for the individual task; **Resume Graph** / **Suspend Graph** buttons operate on the entire graph — suspend order is root-first then all descendants and finalizer task(s); resume order is leaves-first then the root (finalizer before root); Resume buttons are disabled when the task has no trigger configured (finalizer tasks are always treated as having a trigger since `FINALIZE` is their trigger); **Create Finalizer Task…** button is shown for root tasks (no predecessors, not itself a finalizer) and opens the Create Task dialog pre-configured for finalizer mode with FINALIZE pre-filled; disabled with a tooltip if the graph already has a finalizer
  ...
  - **Dependencies** — lists all predecessor tasks; add new predecessors with `ADD AFTER` and remove existing ones with `REMOVE AFTER` per row; **Set as Finalizer For** — assigns the current task as a finalizer for a selected root task; shows a searchable dropdown of root tasks that do not yet have a finalizer assigned; greyed out with a reason when ineligible (has predecessors, has its own schedule, has child tasks, or is already a finalizer for another root task); issues `ALTER TASK … SET FINALIZE = "db"."schema"."root_task"` on confirmation

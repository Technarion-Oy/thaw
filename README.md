...
- **Live SQL diagnostics** — real-time squiggly-line markers appear 400 ms after each edit; no false positives on well-formed Snowflake SQL:
  ...
  - **Semantic warnings** (yellow) — `alias.column` references where the column does not exist in the table's column list (only fires when the column list is already cached from a previous hover or completion; silent when the cache is cold to avoid false positives); example: `SELECT e.nonexistent FROM employees e`

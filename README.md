...
- Right-click an **object** to:
  ...
  - **View Task Graph…** (tasks) — opens an interactive DAG visualisation of the full task graph rooted at the selected task; left-to-right layout is computed automatically via Dagre; each node shows the task name, schedule state (STARTED / SUSPENDED), last-run state badge (Running, Succeeded, Failed, Skipped, Scheduled, Cancelled, Waiting…), and — for completed or failed runs — a completion timestamp (HH:MM:SS for runs today, "Jan 15 HH:MM" for earlier dates); the graph polls Snowflake every 3 seconds and updates node states in place without re-running the layout; a pulsing **Live** indicator and timestamp are shown in the canvas; tasks skipped because a predecessor failed are inferred and shown as Skipped with transitive propagation — correctly overrides a stale Succeeded row from a previous run when the predecessor's failure is more recent; timestamps are suppressed for Skipped nodes since no TASK_HISTORY row exists for the current run; two action buttons are embedded in the top-right of the canvas:
    ...
    - **Right-click any node** to Suspend or Resume that individual task (`ALTER TASK IF EXISTS … SUSPEND/RESUME`); the schedule state badge updates immediately without waiting for the next poll

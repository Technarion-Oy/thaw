...
- Right-click an **object** to:
  ...
  - **Import Data…** (tables) — import one or more local files into a Snowflake table via a temporary internal stage; supports CSV, JSON, AVRO, ORC, and PARQUET; all Snowflake `FORMAT_TYPE_OPTIONS` are exposed with defaults pre-filled in a collapsible panel; the file picker filters to the selected format's extensions; supports two modes:
    ...
    - **AI Suggest** (CSV and JSON, requires AI configured) — an **✨ AI Suggest** button appears in the Format options panel header; clicking it sends a sample of the first file (up to 64 KB) to the configured AI provider, which returns suggested values for delimiter, header detection, quoting, encoding, compression, and other format options; the options are applied automatically and the panel opens to show the changes; a one-sentence explanation from the AI is shown below the panel; an ⓘ info icon next to the button shows a data-privacy notice ("a sample of your file content is sent to your configured AI provider; no data is stored by Thaw")

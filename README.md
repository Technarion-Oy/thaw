...
- Right-click an **object** to:
  ...
  - **View Task Graph…** (tasks) — opens an interactive DAG visualisation of the full task graph rooted at the selected task; left-to-right layout is computed automatically via Dagre; each node shows the task name, schedule state (STARTED / SUSPENDED), and last-run state badge (Running, Succeeded, Failed, Skipped, Scheduled, Cancelled, Waiting…); the graph polls Snowflake every 3 seconds and updates node states in place without re-running the layout; a pulsing **Live** indicator and timestamp are shown in the canvas; tasks skipped because a predecessor failed are inferred and shown as Skipped with transitive propagation; two action buttons are embedded in the top-right of the canvas:

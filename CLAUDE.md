...
Loading state for all these node types is tracked in the `loadingGitNodes` Set (shared, keys are namespaced so no collision). Helper functions `buildStageEntryNodes`, `buildDbtEntryNodes`, and `emptyChildNode` mirror the git equivalents. Backend IPC methods `ExecuteStageFile` and `ListDbtProjectEntries` delegate to existing client methods (`ExecuteGitFile`, `ListStageEntries` respectively) since the underlying SQL patterns are identical. Stage file Execute File is restricted to `.sql` files only; DBT project files do not have an Execute File action.
